...
class DorisSource(Source):
    ...
    def unload_data(self, task):
        ...
        unload_sql = f"SELECT * FROM {task.name} {predicated} INTO OUTFILE \"s3://{object_storage_conf['bucket']}/{file_path}\" \n" \
                     ...
                     f"\"schema\"=\"{self.gen_parquet_schema(task.name.split('.')[0], task.name.split('0')[1])}\"\n" \
                     f");"
    ...
    def gen_parquet_schema(self, database_name, table_name):
        columns = self.get_table_columns(database_name, table_name)
        schema = None
        for index, column in enumerate(columns):
            column_required = "required" if column.is_null else "optional"
            column_type = get_parquet_type(column.type)
            if index == 0:
                schema = f"{column_required},{column_type},{column.name}"
            else:
                schema += f";{column_required},{column_type},{column.name}"
        return schema


def get_parquet_type(data_type):
    if data_type.upper() == "BOOLEAN":
        return "boolean"
    elif data_type.upper() == "INT" or data_type.upper() == "TINYINT" or data_type.upper() == "SMALLINT":
        return "int32"
