...
class ClickZettaDestination(Destination):
    ...
    def load_external_data(self, file_path, schema_name, table_name):
        ...
        load_cmd = f"{meta_cmd_path} append-table -i {self.instance_id} -n {self.config.get('workspace')}" \
                   f",{schema_name} -t {table_name} -r \"{object_file_path}\" -c {self.meta_conf_path}"
        logger.info(
            f"Clickzetta begin to load external data {file_path} to {schema_name}.{table_name} with command: {load_cmd}")
        try:
            os.popen(load_cmd)
