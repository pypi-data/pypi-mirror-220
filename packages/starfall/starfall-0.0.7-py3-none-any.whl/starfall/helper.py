...
def read_log_line(param):
    ...
    log_file = os.path.join(
        setting.runtime_log_path, 'jobhandler', date_str, f'{log_id}.log')
    print(log_file)
    log_list = ['']
...
class JobLogger(object):
    ...
    def __init__(self, jobHandler, logId, level='info'):
        ...
        JOB_LOG_PATH = os.path.join(
            setting.runtime_log_path, 'jobhandler', datestr)
        if os.path.exists(JOB_LOG_PATH) == False:
            os.makedirs(JOB_LOG_PATH)
        th = handlers.TimedRotatingFileHandler(
            filename=setting.runtime_log_path + os.path.sep + "jobhandler" + os.path.sep +
