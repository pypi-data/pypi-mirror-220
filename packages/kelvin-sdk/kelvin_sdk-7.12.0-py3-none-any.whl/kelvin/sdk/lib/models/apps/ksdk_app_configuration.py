...
class ProjectType(BaseEnum):
    ...
    def app_types(cls) -> List:
        ...
        """
        return [cls.kelvin, cls.bridge, cls.docker]
    ...
    def app_types_as_str(cls) -> List[str]:
        ...
        """
        return [cls.kelvin.value, cls.bridge.value, cls.docker.value]

    def project_class_name(self) -> str:
        return_result: str = "project"
        if self in [ProjectType.kelvin, ProjectType.docker, ProjectType.bridge]:
...
class App(KSDKModel):
    ...
    def app_type_configuration(self) -> Optional[Union[KelvinAppType, BridgeAppType]]:
        if self.type == ProjectType.kelvin:
            return self.kelvin
...
class Environment(KSDKModel):
    ...
    def default_environment_configuration(node_name: str, app_name: str) -> dict:
        return Environment(node_name=node_name, workload_name=app_name).dict()  # nosec
