...
class DynamicType:
    @staticmethod
    def fromChar(logger, char) -> tuple[int, str]:
        if len(char) != 1:
        ...
        return (-1, logger.customization.setup_custom_border(char[0]))
    
    @staticmethod
    def fromColoredChar(logger, color, char) -> tuple[int, str]:
        if len(char) != 1:
            MildError.throw(logger, "sagelogs.errors.TooLongChars", "Char in DynamicType is only allowed to have one character! If you need more characters, use \"custom\" method")
        return (-1, logger.customization.setup_custom_border(color + char[0]))
...
class xxPartType:
    ...
    Id : int = 0
    customization : str = ""
    value : tuple[int, str] = (-2, "")
    def __init__(self, name, Id, customization) -> None:
        self.name = name
        self.Id = Id
        self.customization = customization
        self.value = (self.Id, self.customization)

class xxType:
    DEFAULT : xxPartType = xxPartType("DEFAULT", -2, "")
    POSITIVE : xxPartType = xxPartType("POSITIVE", -2, "")
    ONHOLD : xxPartType = xxPartType("ONHOLD", -2, "")
    NEGATIVE : xxPartType = xxPartType("NEGATIVE", -2, "")
    FROZEN : xxPartType = xxPartType("FROZEN", -2, "")
    INFORMATION : xxPartType = xxPartType("INFORMATION", -2, "")
    MILD_EXCEPTION : xxPartType = xxPartType("MILD_EXCEPTION", -2, "")
    def refresh(self, customization):
        self.DEFAULT = xxPartType("DEFAULT", 0, "")
        self.POSITIVE = xxPartType("POSITIVE", 1, customization.setup_custom_border(colorama.Fore.GREEN + "+"))
        self.ONHOLD = xxPartType("ONHOLD", 2, customization.setup_custom_border(colorama.Fore.YELLOW + "/"))
        self.NEGATIVE = xxPartType("NEGATIVE", 3, customization.setup_custom_border(colorama.Fore.RED + "-"))
        self.FROZEN = xxPartType("FROZEN", 4, customization.setup_custom_border(colorama.Fore.LIGHTBLUE_EX + "#"))
        self.INFORMATION = xxPartType("INFORMATION", 5, customization.setup_custom_border(colorama.Fore.CYAN + "i"))
        self.MILD_EXCEPTION = xxPartType("MILD_EXCEPTION", 6, customization.setup_custom_border(colorama.Fore.LIGHTRED_EX + "X"))
        
    def __iter__(self):
        return [self.DEFAULT, self.POSITIVE, self.ONHOLD, self.NEGATIVE, self.FROZEN, self.INFORMATION, self.MILD_EXCEPTION]
...
class SageLogger:
    ...
    def log(self, message : str, type : Union[tuple[int, str], xxPartType] = Type.DEFAULT.value, color = colorama.Fore.RESET, id : int = -2137, date : bool = False, time : bool = False, datecolor : str = colorama.Fore.RESET, timecolor : str = colorama.Fore.RESET, showname : bool = True, ending : str = "\n"):
        typefinish = None
        if(isinstance(type, xxPartType)):
            typefinish = (type.Id, type.customization)
        else:
            typefinish = type
        t = ""
        if date or time:
    ...
    def ask(self, message : str, type : Union[tuple[int, str], xxPartType] = Type.DEFAULT.value, color = colorama.Fore.RESET, answercolor = colorama.Fore.RESET, id : int = -2137, date : bool = False, time : bool = False, datecolor : str = colorama.Fore.RESET, timecolor : str = colorama.Fore.RESET):
        typefinish = None
        if(isinstance(type, xxPartType)):
            typefinish = (type.Id, type.customization)
        else:
