...
class mainGuisaxs(QtGui.QMainWindow):
    def __init__(self, parent=None,splashScreen=None):
        ...
        for name in rec:
            ...
            action.triggered[()].connect(lambda item=item: self.OnRecentFile(item))
                ...
                try:
                    ...
                    for c in cl:
                        try:
                            ...
                            action.triggered[()].connect(lambda item=item: self.callPlugin(item))
                            if item.toolbar:
        ...
        for id in range(len(dd)):
            ...
            item=dd[id][1]
            action.triggered.connect(lambda item=item: self.callModel(item))
        ...
        for name in self.docs:
           ...
           item=name
           action.triggered.connect(lambda item=item: self.OnOpenDocument(item))
    ...
    def OnEditAddReference(self):
        ...
        num,ok = QtGui.QInputDialog.getText(self,"Add a reference value :","specify a value")
