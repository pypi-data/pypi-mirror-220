...
class dlgAbsolute(QtGui.QDialog):#,dlgAbsoluteIui.Ui_dlgSAXSAbsolute):
    def __init__(self,parent,saxsparameters=None,datasetname=None,printout=None,\
        ...
        if self.params is None:
            ...
            if filetools.fileExist(newfn):
                ...
                newfn=filetools.getFilenameOnly(self.workingdirectory+os.sep+filetools.getFilename(self.parentwindow.data_dict[self.datasetname].filename))
                newdataname=newfn
                newfn+='.rpt'
                print 'trying : ',newfn
                if filetools.fileExist(newfn):
                    self.params.getfromRPT(newfn)
                    self.parentwindow.data_dict[self.datasetname].filename=newdataname
                else:
                    print "filename rpt ",newfn," not found" 
    ...
    def ConstructUI(self):
        ...
        for name in paramslist:
            ...
            self.listStaticText[name] = QtGui.QLabel(par.description+" : ",self.ui.groupBox)
            self.listStaticText[name].setAlignment(QtCore.Qt.AlignLeft|QtCore.Qt.AlignVCenter)
            self.ui.formLayout.setWidget(i, QtGui.QFormLayout.LabelRole, self.listStaticText[name])
            self.listTextCtrl[name]=QtGui.QLineEdit(str(par.value),self.ui.groupBox)
            self.ui.formLayout.setWidget(i, QtGui.QFormLayout.FieldRole, self.ui.listTextCtrl[name])
        ...
        if self.datasetlist is not None:
            self.ui.buttonBox.setStandardButtons(QtGui.QDialogButtonBox.Cancel|QtGui.QDialogButtonBox.Close|QtGui.QDialogButtonBox.YesToAll)
