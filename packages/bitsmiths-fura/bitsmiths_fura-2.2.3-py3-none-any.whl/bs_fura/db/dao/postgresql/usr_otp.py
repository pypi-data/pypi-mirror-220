            ...
            while _dex >= 0:
                try:
                    ...
                    if not _qry.fetch():
                        ...
                        raise mettle.lib.xMettle(f"Lock not found: [table:dUsrOtp, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
...
_lock4update.retrys, _lock4update.mili_seconds), err_code = mettle.lib.xMettle.eCode.DBLockNoWaitFailed)
    ...
    def select_one(self) -> "dUsrOtp":
        ...
        if not self.try_select_one():
            raise mettle.lib.xMettle(f"Not found: [table:dUsrOtp, row:{self.rec}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    def select_one_deft(self,
        ...
        if not self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dUsrOtp, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    def select_one_by_key(self, _key: tUsrOtpKey) -> "dUsrOtp":
        ...
        """
        if not self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dUsrOtp, row:{self.irec}]", xMettle.eCode.DBNotFound)
