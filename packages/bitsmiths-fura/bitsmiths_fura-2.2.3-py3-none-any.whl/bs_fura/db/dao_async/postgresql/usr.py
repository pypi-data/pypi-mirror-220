...
class dUsr:
    ...
    async def lock_one_by_key(self, _key: tUsrKey, _lock4update: mettle.db.DBLock, _expected: bool = True) -> bool:
        ...
        :return: True if selected, False if not found and _expected is set to False
        """
        async with dUsrLockOne(self._dbcon) as _qry:
            ...
            while _dex >= 0:
                try:
                    ...
                    if not await _qry.fetch():
                        ...
                        raise mettle.lib.xMettle(f"Lock not found: [table:dUsr, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
...
_lock4update.retrys, _lock4update.mili_seconds), err_code = mettle.lib.xMettle.eCode.DBLockNoWaitFailed)
    ...
    async def try_select_one_by_key(self, _key: tUsrKey) -> bool:
        ...
        """
        async with dUsrSelectOne(self._dbcon) as _qry:
            await _qry.exec(_key)
            if not await _qry.fetch():
                return False
    ...
    async def select_one(self) -> "dUsr":
        ...
        if not await self.try_select_one():
            raise mettle.lib.xMettle(f"Not found: [table:dUsr, row:{self.rec}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_deft(self,
        ...
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dUsr, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_by_key(self, _key: tUsrKey) -> "dUsr":
        ...
        """
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dUsr, row:{self.irec}]", xMettle.eCode.DBNotFound)
    ...
    async def update(self, irec: tUsr = None):
        ...
        async with dUsrUpdate(self._dbcon) as _qry:
    ...
    async def insert(self, irec: tUsr = None):
        ...
        async with dUsrInsert(self._dbcon) as _qry:
            await _qry.exec(self.rec)
    ...
    async def delete_one(self, irec: tUsrKey = None):
        ...
        async with dUsrDeleteOne(self._dbcon) as _qry:
            await _qry.exec(_key)
