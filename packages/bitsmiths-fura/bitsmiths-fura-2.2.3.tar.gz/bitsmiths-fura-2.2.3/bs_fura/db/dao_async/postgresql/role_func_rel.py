...
class dRoleFuncRel:
    ...
    async def lock_one_by_key(self, _key: tRoleFuncRelKey, _lock4update: mettle.db.DBLock, _expected: bool = True) -> bool:
        ...
        :return: True if selected, False if not found and _expected is set to False
        """
        async with dRoleFuncRelLockOne(self._dbcon) as _qry:
            ...
            while _dex >= 0:
                try:
                    ...
                    if not await _qry.fetch():
                        ...
                        raise mettle.lib.xMettle(f"Lock not found: [table:dRoleFuncRel, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
...
_lock4update.retrys, _lock4update.mili_seconds), err_code = mettle.lib.xMettle.eCode.DBLockNoWaitFailed)
    ...
    async def try_select_one_by_key(self, _key: tRoleFuncRelKey) -> bool:
        ...
        """
        async with dRoleFuncRelSelectOne(self._dbcon) as _qry:
            await _qry.exec(_key)
            if not await _qry.fetch():
                return False
    ...
    async def select_one(self) -> "dRoleFuncRel":
        ...
        if not await self.try_select_one():
            raise mettle.lib.xMettle(f"Not found: [table:dRoleFuncRel, row:{self.rec}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_deft(self,
        ...
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dRoleFuncRel, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_by_key(self, _key: tRoleFuncRelKey) -> "dRoleFuncRel":
        ...
        """
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dRoleFuncRel, row:{self.irec}]", xMettle.eCode.DBNotFound)
    ...
    async def insert(self, irec: tRoleFuncRel = None):
        ...
        async with dRoleFuncRelInsert(self._dbcon) as _qry:
    ...
    async def delete_one(self, irec: tRoleFuncRelKey = None):
        ...
        async with dRoleFuncRelDeleteOne(self._dbcon) as _qry:
            await _qry.exec(_key)
