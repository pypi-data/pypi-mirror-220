...
class dConfig:
    ...
    async def lock_one_by_key(self, _key: tConfigKey, _lock4update: mettle.db.DBLock, _expected: bool = True) -> bool:
        ...
        :return: True if selected, False if not found and _expected is set to False
        """
        async with dConfigLockOne(self._dbcon) as _qry:
            ...
            while _dex >= 0:
                try:
                    ...
                    if not await _qry.fetch():
                        ...
                        raise mettle.lib.xMettle(f"Lock not found: [table:dConfig, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
...
_lock4update.retrys, _lock4update.mili_seconds), err_code = mettle.lib.xMettle.eCode.DBLockNoWaitFailed)
    ...
    async def try_select_one_by_key(self, _key: tConfigKey) -> bool:
        ...
        """
        async with dConfigSelectOne(self._dbcon) as _qry:
            await _qry.exec(_key)
            if not await _qry.fetch():
                return False
    ...
    async def select_one(self) -> "dConfig":
        ...
        if not await self.try_select_one():
            raise mettle.lib.xMettle(f"Not found: [table:dConfig, row:{self.rec}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_deft(self,
        ...
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dConfig, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_by_key(self, _key: tConfigKey) -> "dConfig":
        ...
        """
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dConfig, row:{self.irec}]", xMettle.eCode.DBNotFound)
    ...
    async def update(self, irec: tConfig = None):
        ...
        async with dConfigUpdate(self._dbcon) as _qry:
    ...
    async def insert(self, irec: tConfig = None):
        ...
        async with dConfigInsert(self._dbcon) as _qry:
            await _qry.exec(self.rec)
    ...
    async def delete_one(self, irec: tConfigKey = None):
        ...
        async with dConfigDeleteOne(self._dbcon) as _qry:
            await _qry.exec(_key)
