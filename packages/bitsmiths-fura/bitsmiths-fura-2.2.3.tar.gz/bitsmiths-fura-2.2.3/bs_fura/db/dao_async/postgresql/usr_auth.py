...
class dUsrAuth:
    ...
    async def lock_one_by_key(self, _key: tUsrAuthKey, _lock4update: mettle.db.DBLock, _expected: bool = True) -> bool:
        ...
        :return: True if selected, False if not found and _expected is set to False
        """
        async with dUsrAuthLockOne(self._dbcon) as _qry:
            ...
            while _dex >= 0:
                try:
                    ...
                    if not await _qry.fetch():
                        ...
                        raise mettle.lib.xMettle(f"Lock not found: [table:dUsrAuth, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
...
_lock4update.retrys, _lock4update.mili_seconds), err_code = mettle.lib.xMettle.eCode.DBLockNoWaitFailed)
    ...
    async def try_select_one_by_key(self, _key: tUsrAuthKey) -> bool:
        ...
        """
        async with dUsrAuthSelectOne(self._dbcon) as _qry:
            await _qry.exec(_key)
            if not await _qry.fetch():
                return False
    ...
    async def select_one(self) -> "dUsrAuth":
        ...
        if not await self.try_select_one():
            raise mettle.lib.xMettle(f"Not found: [table:dUsrAuth, row:{self.rec}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_deft(self,
        ...
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dUsrAuth, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_by_key(self, _key: tUsrAuthKey) -> "dUsrAuth":
        ...
        """
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dUsrAuth, row:{self.irec}]", xMettle.eCode.DBNotFound)
    ...
    async def update(self, irec: tUsrAuth = None):
        ...
        async with dUsrAuthUpdate(self._dbcon) as _qry:
    ...
    async def insert(self, irec: tUsrAuth = None):
        ...
        async with dUsrAuthInsert(self._dbcon) as _qry:
            await _qry.exec(self.rec)
    ...
    async def delete_one(self, irec: tUsrAuthKey = None):
        ...
        async with dUsrAuthDeleteOne(self._dbcon) as _qry:
            await _qry.exec(_key)
