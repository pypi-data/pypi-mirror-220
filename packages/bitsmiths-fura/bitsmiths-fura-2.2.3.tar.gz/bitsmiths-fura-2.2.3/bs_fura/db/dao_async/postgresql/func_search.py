  ...
  [criteria]""")
    ...
    async def fetch(self, must_exist: bool = False) -> bool:
        ...
        if not await self._dbcon.fetch(self._dbstmnt):
            if must_exist:
                raise mettle.lib.xMettle(f"Record not found: [table:Func, proc:Search, row:{self.irec}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
            return False
    ...
    async def fetch_all(self, out_list: tFunc.List, clear_list: bool = True) -> int:
        """
