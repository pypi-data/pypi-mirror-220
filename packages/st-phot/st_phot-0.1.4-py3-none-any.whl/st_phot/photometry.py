...
class observation3(observation):
    ...
    def upper_limit(self,nsigma=3,method='psf'):
        ...
        if self.telescope.lower()=='jwst':
            ...
            fluxerr = JWST_mag_to_flux(mag,self.wcs,density=False)/nsigma
        else:
            _,_,mag,_,_ = calibrate_HST_flux(tab['fluxerr']*nsigma,1,self.prim_header,self.sci_header)
            fluxerr = HST_mag_to_flux(mag,self.prim_header,self.sci_header)/nsigma
        return mag,fluxerr
...
class observation2(observation):
    ...
    def upper_limit(self,nsigma=3,method='psf'):
        ...
        if self.telescope.lower()=='jwst':
            ...
            fluxerr = JWST_mag_to_flux(mag,self.wcs_list[0],density=False)/nsigma
        else:
            _,_,mag,_,_ = calibrate_HST_flux(tab['fluxerr']*nsigma,1,self.prim_headers[0],self.sci_headers[0])
            fluxerr = HST_mag_to_flux(mag,self.prim_headers[0],self.sci_headers[0])/nsigma
        return mag,fluxerr
