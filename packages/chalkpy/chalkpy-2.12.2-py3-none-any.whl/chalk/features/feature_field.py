...
class Feature(Generic[_TPrim, _TRich]):
    ...
    def join(self) -> Optional[Filter]:
        ...
        for f in self.features_cls.__chalk_features_raw__:
            if (
                hasattr(f.typ.parsed_annotation, "_chalk_feature") and
                self.typ.as_features_cls.namespace == f.typ.parsed_annotation._chalk_feature.namespace
            ):
...
"""
    ...
    def _validate_filter(self, filter: Filter):
        if (
                not isinstance(filter, Filter) or
                not isinstance(filter.lhs, Feature) or
                not isinstance(filter.rhs, Feature) or
                filter.operation != "=="
        ):
            raise TypeError(
                f"Join function for {self.root_fqn} is incorrectly configured. "
