...
import { AllocationCloud, Asset, Shares } from '../PiecesSDK/connector';
import { AccessEnum, AllocationStatusEnum } from '../PiecesSDK/core';
    ...
    public async generate({
        ...
        try {
            ...
            if (!profile?.user) {
                ...
                const allocation: AllocationCloud | undefined =
                    await cloud.connect({
                        user: profile?.user,
                    });
                if (!allocation) {
                ...
                profile?.user?.allocation?.status.cloud !=
                AllocationStatusEnum.Running
            ) {
                notifications.error({
