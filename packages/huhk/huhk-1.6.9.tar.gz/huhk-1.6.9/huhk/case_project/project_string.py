...
class ProjectString(ProjectInIt):
    ...
    def get_path(self, name, fun_type="api"):
    ...
    def get_api_fun_fun_str(self, fun_name):
        ...
        if set(api.input) & set(self.page_and_size):
            for n in data_list:
                n_l = re.findall(f'^(.*)({"|".join(self.before)}|{"|".join(self.end)})(.*)$', n, re.I)
                if n_l and (n_l[0][0] and not n_l[0][2]) and (not n_l[0][0] and n_l[0][2]):
