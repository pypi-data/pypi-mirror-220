...
class MetaThing():
    ...
    def MTGenAdapterModel(self):
        ...
        def check_built_in(name):
            return name[0] != '_' and name[0].islower() and name not in ['ping', 'selector', 'release', 'type']
        
        predicate = lambda object: callable(object) and check_built_in(object.__name__)
        
        # App
        # app_fns = [(name, member) for name, member in inspect.getmembers(self.srv.app, predicate=inspect.ismethod) if check_built_in(name)]
        app_fns = [(name, type(member)) for name, member in inspect.getmembers(self.srv.app, predicate=predicate)]
        
        # Dev
        dev_fns = [(name, type(member)) for name, member in inspect.getmembers(self.srv.app.Dev, predicate=predicate)]
        ...
        def model_contains_fn(fn_name, is_event, is_global):
            if len(am) == 0:
                return False
            for item in am['adapterevt' if is_event else 'adapteract']:
        ...
        # Main
        import __main__
        info = get_yaml('Service Info', __main__.__doc__) or {'name': '', 'description': '', 'icon': 'mdi-code-array'}
        message = [
            f"!-= Service Info =-",
            f"Name: {info['name']}",
            f":  Desc: {info['description']}",
            f":  Icon: {info['icon'] if 'icon' in info else ''}",
        ] if info is not None else [f"!-= Service Info (Main.py head comment) not found =-"]
        ...
        def scan_fn_info(fn_config, message, is_event, is_global):
            try:
                ...
                message += [f"@Fn: {fn[0]} (Cannot parse config): {fn_config}"]
            return message
        ...
        # Import configs
        model['init_config'] = missing_model_fields['app_config']
        model['device_init_config'] = missing_model_fields['dev_config']
        model['adapteract'] = missing_model_fields['act']
        model['adapterevt'] = missing_model_fields['evt']
