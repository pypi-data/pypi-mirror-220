...
def plot_estimate():
	...
	Acc = (
		ggplot(df, aes(x="acc")) + geom_density(adjust=10) +
...
def plot_GC_bias():
	...
	distribution_len = (
		ggplot(df, aes(x="GC_content", y="number")) + geom_point(size=1) + geom_line() +
	...
	GC_bias = (
		ggplot(df1, aes(x="GC_content", y="nor_dp")) + geom_point(size=1) + geom_line() +
