...
class Augmenter:
    def __init__(self, seed_data, model_name="lamini/open", prompt = None, related_data=None, schema=None, config={}):
        ...
        if self.batch:
            ...
            for i in tqdm(range(0, n, batch_size)):
                ...
                for _ in range(attempts):
                    try:
                        outs = self.llm(data_to_llm, AugmenterOutput, temperature=0.7, schema=self.schema, task=self.prompt)
            ...
            for i in tqdm(range(n)):
                ...
                for _ in range(attempts):
                    try:
                        out = self.llm(datum_to_llm, AugmenterOutput, temperature=0.7, schema=self.schema, task=self.prompt)
                    except LlamaAPIError as e:
                        print(f"Lamini API error {e}, retrying")
                if not out:
                    raise RuntimeError("Too many Lamini API errors.")
...
class AnswerAugmenter:
    def __init__(self, questions, output_type=None, model_name="lamini/instruct", prompt="", seed_data=None, related_data=None, schema=None, config={}):
    ...
    def run(self, verbose=False):
        if self.batch:
            ...
            for i in tqdm(range(0, len(self.questions), batch_size)):
                ...
                for _ in range(attempts):
                    try:
                        answers = self.llm(data_to_llm, self.output_type, schema=self.schema, task = self.prompt)
                    except LlamaAPIError as e:
                        print(f"Lamini API error {e}, retrying")
                if not answers:
                    raise RuntimeError("Too many Lamini API errors.")
            ...
            for datum in tqdm(self.questions):
                ...
                for _ in range(attempts):
                    try:
                        answer = self.llm(datum_to_llm, self.output_type, schema=self.schema, task = self.prompt)
                    except LlamaAPIError as e:
                        print(f"Lamini API error {e}, retrying")
                if not answer:
                    raise RuntimeError("Too many Lamini API errors.")
...
class QuestionAnswerAugmenter: # TODO: eventually inherit from generic DataAugmenter class
    ...
    def __init__(self, seed_data, question_model_name="lamini/open", answer_model_name="lamini/instruct", question_prompt="", answer_prompt="", related_data=None, schema=None, config={}):
    ...
    def run(self, n, verbose=False):
        augmented_questions = Augmenter(self.seed_data, model_name=self.question_model_name, prompt = self.question_prompt, related_data=self.related_data, schema=self.schema, config=self.config).run(n, verbose=verbose)
        augmented_answers = AnswerAugmenter(augmented_questions, output_type=self.answer_type, prompt = self.answer_prompt, model_name=self.answer_model_name, seed_data=self.seed_data, related_data=self.related_data, schema=self.schema, config=self.config).run(verbose=verbose)
