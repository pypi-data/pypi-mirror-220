...
import importlib
import logging
from mypy.stubgen import find_module_paths_using_imports, IGNORED_DUNDERS
...
class BoostClassInspect:
    ...
    def cls_props(self) -> List[Tuple[str, Any]]:
        return [(n, t.__class__.__name__) for n, t in inspect.getmembers(self.c, lambda o: not callable(o) and not inspect.ismethod(o) and not inspect.isbuiltin(o) and not isinstance(o, property)) if not n.startswith('__') and t.__class__.__name__ != 'class']
    ...
    def constructors(self) -> List[Tuple[str, Tuple[List[str], List[str]]]]:
        ...
        return [(n, CStubGenerator.parse_boost_python_function(func_name=n, func=f, cls_name=self.c.__name__,mod_path_replace=self.module+".")) for n, f in c_meth]
    ...
    def methods(self) -> List[Tuple[str, Tuple[List[str], List[str]]]]:
        ...
        return [(n, CStubGenerator.parse_boost_python_function(func_name=n, func=f, cls_name=self.c.__name__,mod_path_replace=self.module+".")) for n, f in c_meth if n != '__reduce__']
...
class BoostModuleInspect:
    ...
    def functions(self) -> List[Tuple[str, Tuple[List[str], List[str]]]]:
        ...
        return [(n, CStubGenerator.parse_boost_python_function(func_name=n, func=f, cls_name=self.c.__name__,mod_path_replace=self.name+".")) for n, f in c_funcs]
    ...
    def variables(self) -> List[Tuple[str, str]]:
        vars = inspect.getmembers(self.c, lambda o: not inspect.isclass(o) and not callable(o))
        return [(n, t.__class__.__name__) for n, t in vars if not n.startswith('__')]
...
class CStubGenerator:
    ...
    known_signatures: Dict[str, Tuple[str, str]] = {
        ...
        'StmTask.__init__':('self, id: int, name: str, created: time, json: str = "", labels: StringVector =StringVector() , cases: StmCaseVector = StmCaseVector(), base_model: StmModelRef = StmModelRef(), task_name: str = ""','None')
    }

    def __init__(self, mod_path: str = None):
        self.mod_path_replace: str = f'{mod_path}.' if mod_path is not None else None
    ...
    def parse_boost_python_function(func_name: str, func: Any, cls_name: Optional[str] = None, mod_path_replace: Optional[str] = None) -> tuple[list[str], list[str]]:
        ...
        overline_lines = []
        for line in docs:
            if re.match(rf"{func_name}\(.*\) -> [\w]* :", line):
        ...
        for overline_lines in overloads:
            ...
            _utc_not_valid = "[not-valid-period>"
            _time_zero = "time(0)"
            ...
            return_type = "Any" if return_type == "object" else return_type

            overload_sigs.append(f"({start}) -> {return_type}")

        return overload_sigs, overload_docs

    def stubs_per_module(self, f: TextIO, module, force_use_boost_parser: bool = False):
        """ Common/adapted entry for the different modules """
        f.write(f'"""This file is auto-generated with {Path(__file__).name}"""\n')
        f.write("from typing import Any,overload,Callable\n")
        f.write("from enum import Enum\n")
        if module.__name__ != 'shyft.time_series._time_series':  #those needs imports helpers.
            f.write('from shyft.time_series._time_series import *\n')
        if module.__name__ == 'shyft.energy_market.stm._stm':
            f.write('from shyft.energy_market.core._core import StringTimeSeriesDict,GateList,CatchmentList,ModelArea,HydroComponent,HydroComponentList,ConnectionRole,TurbineDescription,XyPointCurve,XyPointCurveWithZ,XyPointCurveWithZList\n')
            f.write('from shyft.energy_market.core import _core as core\n')
            f.write('unit_group_type=UnitGroup.unit_group_type\n')  # hack to fix nested enum
        if module.__name__.startswith('shyft.hydrology.') and module.__name__ != 'shyft.hydrology._api':
            f.write('from shyft.hydrology._api import *\n')  # common for all hydrology modules
        if module.__name__ == 'shyft.energy_market.stm.shop._shop':
            f.write('from shyft.energy_market.stm._stm import StmSystem, LogEntryList\n')
        f.write("# import Boost.Python\n")
        f.write("nan = float('nan')\n")
        sg=BoostModuleInspect(module)
        sg.create_stub(o=f,indent=0)
        # self.stubs_for_obj(f, module, force_use_boost_parser, indent=0)
...
if __name__ == "__main__":
    ...
    for stub_source in c_modules:
        ...
        with fn.open('w') as f:
            ...
            stub_generator = CStubGenerator(stub_source.module)
            stub_generator.stubs_per_module(f, lib)
