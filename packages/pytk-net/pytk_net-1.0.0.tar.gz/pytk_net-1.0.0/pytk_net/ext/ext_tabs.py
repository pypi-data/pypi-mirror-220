...
class NavMenu(Frame):
    def __init__(self, parent, tab_items: List[TabItem], on_selected: Callable, default_color=DARK,
                 hover_color=SECONDARY, selected_color=SECONDARY, icon_txt_color=LIGHT, **kwargs):
...
class ExtTabs(Frame):
    def __init__(self, master, tabs: List[TabItem], width=300, height=800, **kwargs):
        super().__init__(master)
        self.cur_frame = None
        self.nav = NavMenu(master, tab_items=tabs, width=width, height=height, on_selected=self.on_selected, **kwargs)
