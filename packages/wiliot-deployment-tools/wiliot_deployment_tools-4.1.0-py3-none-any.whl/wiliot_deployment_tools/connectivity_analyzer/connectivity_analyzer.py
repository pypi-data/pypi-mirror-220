...
class GwHbActionClient(ExtendedEdgeClient):
    ...
    def check_gw_hb_ack_received(self, gateway_id,payload, time_sent):
        ...
        for line in range(len(acks)):
            ...
            payload  = acks['payload'].iloc[line]
            rssi = int(payload[12:14], 16)
            if brg_mac in rssi_dict:
                ...
                rssi_dict[brg_mac] = rssi  
        acks = acks[(acks['actionType'] == BridgeThroughGatewayAction.GW_HB) &
                    (acks['payload'].apply(lambda x: x[:12])
                     == payload[:12])] 
        if len(acks) > 0:
            return rssi_dict
        return None
    ...
    def send_gw_hb_packet_to_brgs_list(self,bridges,gateways,minutes_timeout=3,rssi_th = 76,brg_gw_dict = None):
        """
    ...
    def bridge_gw_downlink_analyzer(self, location, minutes_timeout=3,bridges_list = None,gws_list = None):
        """
        ...
        unconnected_bridges = self.send_gw_hb_packet_to_brgs_list(bridges = bridges,minutes_timeout=minutes_timeout
                                                                      ,gateways=gateways,rssi_th=rssi_th,brg_gw_dict=brg_gw_dict)
