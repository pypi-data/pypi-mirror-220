...
class Request:
    ...
    def using(self, client):
        ...
        :arg client: an instance of ``elasticsearch.Elasticsearch`` to use or
            an alias to look up in ``elasticsearch_dsl_aq.connections``
...
class MultiSearch(Request):
    """
    Combine multiple :class:`~elasticsearch_dsl_aq.Search` objects into a single
    request.
    """
    ...
    def add(self, search):
        """
        Adds a new :class:`~elasticsearch_dsl_aq.Search` object to the request::
