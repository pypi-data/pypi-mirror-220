...
from Shared.certoraUtils import get_certora_root_directory, read_json_file, remove_file, \
    write_json_file, abs_posix_path
...
class JobList:
    ...
    def __init__(self, current_path: Path = Path.cwd()):
        ...
        self.recent_jobs_path = get_certora_root_directory() / RECENT_JOBS_FILE
        self.temp_file_path = get_certora_root_directory() / f".tmp{RECENT_JOBS_FILE}"
    ...
    def save_data(self) -> None:
        ...
        try:
            ...
            write_json_file(output_data, self.recent_jobs_path)
    ...
    def save_recent_jobs_to_path(self, path: Path = Path.cwd()) -> None:
        """
        ...
        recent_jobs_file_path: Path = path / RECENT_JOBS_FILE
        if path not in all_data:
            ...
            if recent_jobs_file_path.exists():
                # backup
            ...
            try:
                job_logger.debug(f"writing recent jobs file to {abs_posix_path(recent_jobs_file_path)}")
    ...
    def rename_recent_jobs_file(self) -> None:
        ...
        name = get_certora_root_directory() / f".incompatible.{current_time}{RECENT_JOBS_FILE}"
        try:
