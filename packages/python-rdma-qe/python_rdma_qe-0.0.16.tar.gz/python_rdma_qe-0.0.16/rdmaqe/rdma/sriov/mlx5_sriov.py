...
class Mlx5Sriov(BaseSriov):
    ...
    def __init__(self, hca_id):
        super().__init__(hca_id)
        self.total_vf = self.get_total_vf()
        self.num_vf = self.get_num_vf()
    ...
    def create_num_vf(self, num_vfs):
        ...
        """
        if self.num_vf is not None and self.num_vf != num_vfs:
            ...
            if run(cmd):
                ...
                return False
        elif self.num_vf == num_vfs:
            print(f"Info: there already exist {self.num_vf} vfs")
            return True
        else:
            print("ERROR: please check if the device supports this operation.")
            return False
    ...
    def delete_vf(self):
        ...
        if self.num_vf == 0:
            ...
            cmd = f"echo 0 > /sys/class/infiniband/{self.hca_id}/device/sriov_numvfs"
            if run(cmd):
                return True
            else:
                return False
