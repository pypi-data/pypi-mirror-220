...
class WebDbSearchJob(JobBase):
    ...
    def execute(self):
        ...
        # It is important NOT to do this in initialize() because the time window
        # between initialize() and execute() is used to register callbacks. If
        # we search before all callbacks are registered, signals are ignored.
...
class _Searcher:
    ...
    def search(self, query):
        ...
        if self._current_query != query:
            ...
            self._current_query = query.copy()
    ...
    async def _search(self, query):
        ...
        # TODO: Disabled for now. If this doesn't cause any issues, _delay()
        #       below should be removed. Remember to also remove the disabled
        #       tests!
        # await self._delay()
        results = ()
        try:
    ...
    #         _log.debug('... sleeping %s seconds', remaining_delay)
