...
@fixture
def potentially_malicious_pytorch_weights_file(tmp_path: Path) -> Path:
    """Returns a path to a potentially malicious PyTorch weights file.
    ...
    class FakeModelFile:
        ...
        def __reduce__(self) -> Tuple[Callable, Tuple[str]]:
            cmd = f"touch {str(tmp_path)}/empty_file.txt"
            return os.system, (cmd,)

    with open(tmp_path / "fakemodel.pickle", "wb") as f:
        pickle.dump(FakeModelFile(), f)

    return tmp_path / "fakemodel.pickle"
