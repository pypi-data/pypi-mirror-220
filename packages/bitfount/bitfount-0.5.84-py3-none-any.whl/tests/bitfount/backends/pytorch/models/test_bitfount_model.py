...
class TestPyTorchBitfountModel:
    ...
    def test_deserialization_is_safe(
        ...
        datastructure: DataStructure,
        dummy_model_class: Type[PyTorchBitfountModel],
        potentially_malicious_pytorch_weights_file: Path,
    ) -> None:
        ...
        parent_dir = potentially_malicious_pytorch_weights_file.parent
        # Check that the file does not exist before deserialization.
        assert not (parent_dir / "empty_file.txt").exists()
        ...
        try:
            model.deserialize(
                potentially_malicious_pytorch_weights_file,
            )
        except Exception:
        ...
        # Asserts that the arbitrary code in the file is now executed because
        # `weights_only` is set to False.
        try:
            model.deserialize(
                potentially_malicious_pytorch_weights_file, weights_only=False
            )
        except Exception:
            pass  # This is expected
        else:
        ...
        parent_dir = potentially_malicious_pytorch_weights_file.parent
        assert (parent_dir / "empty_file.txt").exists()
