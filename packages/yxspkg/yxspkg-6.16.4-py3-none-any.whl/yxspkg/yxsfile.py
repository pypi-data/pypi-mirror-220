...
class yxsFile:
    ...
    def to_pureFile(self,to_dir = None,output=None,force=False):
        if not self.is_pureFile:
            fp   = open(self.file_name_origin,'rb')
    ...
    def play_video(self):
        os.chdir(self.file_name_origin.absolute().parent)
        url = 'http://0.0.0.0:9090/file_downloader/x./'+quote(self.file_name_origin.name)
        ...
        if stat == 0:
            ...
            td.join()
    def view_image(self):
        songziviewer.main(self.file_name_origin)
    ...
    def read(self,BUFFER_SIZE=-1):
        if not self._fp:
            self._fp = open(self.file_name_origin,'rb')
            if not self.is_pureFile:
    ...
    def file_size(self):
        if self.is_pureFile:
            return self.file_name_origin.stat().st_size
        else:
            stem = self.file_name_origin.stem
            if stem.endswith('.3'):
                t = open(self.file_name_origin,'rb').read(self.title_size)
                head_info = parse_header(t,self.passwd)
                size = head_info['len']
            else:
                size = self.file_name_origin.stat().st_size - self.title_size
            return size
    ...
    def get_head_info(self):
        if self._head_info is None:
            tts = open(self.file_name_origin,'rb').read(self.title_size)
            if self.file_name_origin.stem[-2] != '.':
                self.passwd = self.passwd_v1
            self._head_info = parse_header(tts,self.passwd)
