...
class Classification:
    ...
    def Voting_Classifier(self, models:list, ordinal_cat_cols=None, scoring='accuracy',
        ...
        if grid_params is not None and random_params is None and bayes_pbounds is None:
            assert all('Voting__' + m[0] + '__' in s for s in grid_params.keys() for m in models), "Grid parameters must start with 'Voting__modelname__'"
        elif random_params is not None:
            assert all('Voting__' + m[0] + '__' in s for s in random_params.keys() for m in models), "Random parameters must start with 'Voting__modelname__'"
        elif bayes_pbounds is not None:
            assert all('Voting__' + m[0] + '__' in s for s in bayes_pbounds.keys() for m in models), "Bayesian parameter bounds must start with 'Voting__modelname__'"
            assert all('Voting__' + m[0] + '__' in s for s in bayes_int_params for m in models), "Bayesian integer parameters must start with 'Voting__modelname__'"
