...
class NowPlayingRsEzooGraph(GetItemGraph):
    def __init__(self, e_graph=None, _graph=None, name=None, _dataset=None):
        super(NowPlayingRsEzooGraph, self).__init__(e_graph=e_graph, _graph=_graph, name=name)
...
class NowPlayingRsEzooGraphhDataset(EzooHeteroGraphDataset):
    
    def __init__(self, name=EzooExampleDatasetEnum.NOWPLAYINGRS.value, rpc_url=None, raw_dir=None, force_reload=False, cfg_file=None, gdi_ptr=0, restore_file='', restore_url='', node_exclude_list=['id'], edge_exclude_list=['_ID', 'id', 'edgeIds', 'listened-by_id', 'created_at'], batch_size=0,
                 split_ratio=None, data_path=None, force_download=False):
        feat_mapping = {'node':
                            {'track':
                                 {'song_features':
                                               ['song_features0', 'song_features1', 'song_features2', 'song_features3',
                                                     'song_features4', 'song_features5', 'song_features6', 'song_features7', 'song_features8']
                                  }
                             }, 'edge': {}
                        }
        self.batch_size = batch_size
        super(NowPlayingRsEzooGraphhDataset, self).__init__(name, rpc_url, raw_dir, force_reload, cfg_file, gdi_ptr, restore_file, restore_url, node_exclude_list, edge_exclude_list,
    ...
    def __getitem__(self, item):
        ...
        if item is EzooShapeGraphEnum.WHOLE:
            ...
            return NowPlayingRsEzooGraph(self.e_graph, self.dgl_graph, self.db_name, _dataset)

        return NowPlayingRsEzooGraph(self.e_graph, self.dgl_graph, name=self.db_name)
