...
from ezoognn.loader.dataset.gas_ezoo_data_graph import PolEzooGraphDataset
...
class TestPolAndBuildDglGraph:
    ...
    def setup_class(self):
        self.pol = PolEzooGraphDataset(cfg_file=cfg_file, force_download=True)[EzooShapeGraphEnum.WHOLE][0]
        self.dgl_graph = GASDataset('pol')[0]
    ...
    def test_nodeTypes(self):
        assert(len(self.dgl_graph.ntypes) == len(self.pol.ntypes))
        assert(set([True for nt in self.dgl_graph.ntypes if nt in self.pol.ntypes]), {True})

    def test_edgeTypes(self):
        assert(len(self.dgl_graph.etypes), len(self.pol.etypes))
        assert(set([True for nt in self.dgl_graph.etypes if nt in self.pol.etypes]), {True})

    def test_nodeSize(self):
        assert(self.pol.number_of_nodes() == self.dgl_graph.number_of_nodes())

    def test_edgeSize(self):
        assert(self.pol.number_of_edges() == self.dgl_graph.number_of_edges())

    def test_nodeShape(self):
        for nt in self.dgl_graph.ntypes:
            assert(self.dgl_graph.nodes(nt).shape == self.pol.nodes(nt).shape)
    def test_node_feat_props(self):
        assert(self.pol.nodes['u'].data['feat'].shape == self.dgl_graph.nodes['u'].data['feat'].shape)

    def test_edgeShape(self):
        assert(len(self.dgl_graph.edata._etype) == len(set(self.dgl_graph.edata._etype + self.pol.edata._etype)))
        for key in self.dgl_graph.edata:
            value1 = self.dgl_graph.edata[key]
            value2 = self.pol.edata[key]
            for kk in value1:
                vv1 = value1[kk]
                vv2 = value2[kk]
                assert(vv1.shape == vv2.shape)

    def test_edge_mask_props(self):
        assert(self.pol.edges['forward'].data["train_mask"].nonzero().squeeze().shape == self.dgl_graph.edges['forward'].data["train_mask"].nonzero().squeeze().shape)
