...
from ezoognn.loader.dataset.ogb_ezoo_data_graph import OgbnEzooHerteroGraphDataset
...
def parse_args():
    ...
    parser.add_argument("-cfg", "--cfg-file", type=str, default='/opt/code/c++/local-ezoodb/ezoodb/resources/conf/ezoodb.conf',
...
def prepare_data(args):
    if args.__contains__('ezoo_fullgraph') and args['ezoo_fullgraph']:
        start = time.time()
        # 获取异构图；graph: dgl graph object, label: torch tensor of shape (num_nodes, num_tasks)
        _, g, split_idx, label_dict = OgbnEzooHeteroGraphDataset(name=EzooExampleDatasetEnum.OGBN_MAG, cfg_file=args['cfg_file'])[EzooShapeGraphEnum.WHOLE]
        split_idx = split_idx['node_mask']
        label_dict = label_dict['nlabel']
        labels = label_dict['paper'].long().flatten()
