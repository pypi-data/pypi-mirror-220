# 代码说明
# 参考：https://github.com/Lightning-AI/lightning/issues/8316
# 实现 inter-batch-parallelism，但未达到预期效果
# 更进一步的研究可参考：https://github.com/Lightning-AI/lightning/blob/08f78604bbaa5a7c65fb8a9ac852451cbeef1653/pytorch_lightning/utilities/dataloader_fetcher.py
...
class StoTrainWithEzooSamplInterBatchParallelism(LoadingWholeGraph):
    ...
    def load_subtensor(self, nfeat, labels, seeds, input_nodes, device):
        ...
        """
        batch_inputs = nfeat[input_nodes].to(device=device, non_blocking=True)
        batch_labels = labels[seeds].to(device=device, non_blocking=True)
    ...
    def train(self):
        ...
        for epoch in range(self.args.n_epochs):
            ...
            batch_i_ready = torch.cuda.Event()

            # Route the first batch and kick off the routing for the second batch
            input_nodes, seed_nodes, blocks = next(data_iter)
            with torch.cuda.stream(comm_stream):
                batch_inputs, batch_labels = self.load_subtensor(self.features, self.labels, seed_nodes, input_nodes, device)
                blocks = [block.int().to(device=self.device, non_blocking=True) for block in blocks]
                batch_i_ready.record()
            ...
            while not done:
                ...
                try:
                    ...
                    if self.args.prof >= 0: torch.cuda.nvtx.range_push("dataloader.next()")
                    input_nodes_ip1, seed_nodes_ip1, blocks_ip1 = next(data_iter)
                ...
                with torch.cuda.stream(comm_stream):
                    ...
                    # try:
                    if self.args.prof >= 0: torch.cuda.nvtx.range_push("load_subtensor&to_block")
                    batch_inputs_ip1, batch_labels_ip1 = self.load_subtensor(self.features, self.labels, seed_nodes_ip1,
                                                                     input_nodes_ip1, device)
                    blocks_ip1 = [block.int().to(device=self.device, non_blocking=True) for block in blocks_ip1]
                    if self.args.prof >= 0: torch.cuda.nvtx.range_pop()
                ...
                #     tic_todev = time.time()
                #     input_nodes, seed_nodes, blocks = next(data_iter)
                #     batch_inputs, batch_labels = self.load_subtensor(self.features, self.labels, seed_nodes,
                #                                                      input_nodes, device)
                #     blocks = [block.int().to(device=self.device, non_blocking=True) for block in blocks]
                #     toc_todev = time.time()
                # except StopIteration:
                ...
                # 加上下面这个同步语句之后，train和dataloader就会同步，否则会overlap
                # torch.cuda.synchronize()
                toc_train = time.time()

                batch_inputs = batch_inputs_ip1
                batch_labels = batch_labels_ip1
                blocks = blocks_ip1
                batch_i_ready = batch_ip1_ready

                print('Step {:05d} | On Loader {:06f} | On ToDev {:06f} | On Train {:06f} | On Iter {:06f}'.format(
                      iter_id,
                      toc_loader - tic_loader,
                      toc_todev - tic_todev,
                      toc_train - tic_train,
                      toc_train - tic_step))
                tic_step = time.time()
            ...
            #     # if self.args.prof >= 0: torch.cuda.nvtx.range_pop()
            #     #
            #     # if self.args.prof >= 0 and iter_id == self.args.prof + 5:
            #     #     print("Profiling ended at iteration {}".format(iter_id))
            #     #     torch.cuda.cudart().cudaProfilerStop()
            #     #     quit()
            # ========================================
