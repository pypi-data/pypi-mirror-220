...
class Node2vec(nn.Module):
    ...
    def sample(self, batch):
        ...
        if self.args.sampler == 'ezoo':
            pos_traces = ezoo_node2vec_random_walk(
                self.g, self.args.e_graph, batch, self.p, self.q, self.walk_length, self.prob)
        else:
            pos_traces = node2vec_random_walk(
                self.g, batch, self.p, self.q, self.walk_length, self.prob)

        pos_traces = pos_traces.unfold(
            1, self.window_size, 1)  # rolling window
        ...
        for i in range(epochs):
            ...
            if self.eval_steps > 0:
                if epochs % self.eval_steps == 0:
                    ...
                    acc_loger.append(acc)
        # Use val_set instead of test_set 
        acc = self._evaluate_step()
        MetricsReporter.report_final_result(acc)
        return min(loss_loger), max(acc_loger)
