...
def savedata_gwy(gwy_file_path:pathlib.Path, size_info:GwySizeInfo, data_sets:list[np.ndarray], data_labels:list[str], data_units:list[str], meta_data:dict = {}, time_str:str = None) -> bool:
    ...
    try:    
        ...
        for i in range(num_channels):
            ...
            x_axis = np.linspace(size_info.x_offset, size_info.x_offset + size_info.x_range, image_number_of_line)
            y_axis = np.linspace(size_info.y_offset, size_info.y_offset + size_info.y_range, image_points_per_line)
