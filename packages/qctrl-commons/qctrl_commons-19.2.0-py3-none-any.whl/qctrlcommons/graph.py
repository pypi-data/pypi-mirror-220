...
class Graph:
    ...
    def _add_composite_nodes(self):
        ...
        for composite_namespace in COMPOSITE_NAMESPACE_REGISTRY:
            _extend_method(
                self,
                composite_namespace(self),
                composite_namespace.namespace_name,
            )

        for primary_namespace in PRIMARY_NAMESPACES:
            # Create namespace.
            _extend_method(self, primary_namespace, primary_namespace.namespace_name)

            # Bind namespace methods.
            for node_name, node_class in primary_namespace.sub_nodes.items():
                node_method = node_class.create_graph_method()
                _extend_method(
                    getattr(self, primary_namespace.namespace_name),
                    partial(node_method, self),
                    node_name,
                    False,
                )
