...
def dict2obj(dict1):
    ...
    return json.loads(json.dumps(dict1), object_hook=params)
...
def image_demo(predictor, vis_folder, current_time, args,exp):
        ...
        if outputs[0] is not None:
            ...
            for t in online_targets:
                ...
                if tlwh[2] * tlwh[3] > args.min_box_area and not vertical:
                    ...
                    # save results
                    results.append(
                            {f"{frame_id:06d}":{"id": tid, "bbox":[int(tlwh[0]),int(tlwh[1]),int(tlwh[2]),int(tlwh[3])],"frame_id":f"{frame_id:06d}"}}
    ...
    if args.save_result:
        ...
        res_file = osp.join(vis_folder, f"output.json")
        output_json=json.dumps({"detections":results})
            ...
            if outputs[0] is not None:
                ...
                for t in online_targets:
                    ...
                    if tlwh[2] * tlwh[3] > args.min_box_area and not vertical:
                        ...
                        online_scores.append(t.score)
                        results.append(
                            {f"{frame_id}":{"id": tid, "bbox":[int(tlwh[0]),int(tlwh[1]),int(tlwh[2]),int(tlwh[3])],"frame_id":frame_id}}
                            # f"\"{frame_id}\":{tid},{tlwh[0]:.2f},{tlwh[1]:.2f},{tlwh[2]:.2f},{tlwh[3]:.2f},{t.score:.2f},-1,-1,-1\n"
                        )
                timer.toc()
                online_im = plot_tracking(
...
def infer(**kargs):
    # print(kargs)
    params = dict2obj(kargs)
    exp = get_exp(params.exp_file, params.name)

    params.ablation = False
    params.mot20 = not params.fuse_score
