        ...
        with snowflake.connector.connect(
            ...
            try:
                success, nchunks, nrows, _ = write_pandas(conn, df, table_name, parallel=8,schema=schema,database=database,
                                        auto_create_table=auto_create_table,overwrite=overwrite,chuck_size=chunk_size)
                ...
                try:
                    now = time.time()
                    success, nchunks, nrows, _ = write_pandas(df, table_name, parallel=8,schema=schema,database=database,
                                            auto_create_table=auto_create_table,overwrite=overwrite,chuck_size=chunk_size)
                    time_taken = round(time.time() - now,2)
                    print(f"Sent Data to {table_name}")
