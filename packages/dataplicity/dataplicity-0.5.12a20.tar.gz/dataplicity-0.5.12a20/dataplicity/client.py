...
class Client(object):
    ...
    def _init(self):
        try:
            ...
            # we need to load features after possible migration to device_token
            self.load_features()
    ...
    def assign_device_class_from_file(self):
        ...
        headers = {
            "DA_TOKEN": f"{self.device_token}",
            'Content-Type': 'application/json'
        }
        response = contrib.requests_post_json(
            urljoin(self.rpc_url, "/deviceclass/"),
            data=device_class_data,
            headers=headers
        )
    ...
    def load_features(self):
        if not self.device_token:
            log.warning("No device token found. Skip loading features.")
            return
        ...
        if response.ok:
            ...
            log.info("Authentication error meanwhile loading features: [%s] %s", response.status_code, response.data)
    ...
    def migrate_to_device_token(self, serial, auth_token):
        ...
        response = contrib.requests_post_json(
            urljoin(self.rpc_url, "/auth/"),
            data=json_data
        )
        if response.ok:
            self._write(constants.DEVICE_TOKEN_LOCATION, response.data["device_token"])
            self.device_token = response.data["device_token"]
            log.info("New device auth token created")
        else:
            log.info("Authentication error: [%s] %s", response.status_code, response.data)
