...
from hspylib.core.tools.commons import dirname, file_is_not_empty, syserr, sysout
...
class Setman(metaclass=Singleton):
    ...
    HHS_SETMAN_CONFIG_FILE = os.environ.get("HHS_SETMAN_CONFIG_FILE", "setman.properties")

    HHS_SETMAN_DB_FILE = os.environ.get("HHS_SETMAN_DB_FILE", "setman-db")
    ...
    def __init__(self, parent_app: Application) -> None:
        ...
        if not file_is_not_empty(self.HHS_SETMAN_CONFIG_FILE):
            self._setup(self.HHS_SETMAN_CONFIG_FILE)
    ...
    def _source_settings(self, name: str | None, filepath: str | None) -> None:
        ...
        # Used print to avoid any ending vt codes.
        if filepath:
            with open(filepath, 'a', encoding=Charset.UTF_8.val) as f_exports:
                f_exports.writelines(os.linesep.join(self.settings.as_environ(prefixed)))
        else:
            sysout(os.linesep.join(self.settings.as_environ(prefixed)))
    ...
    def _setup(self, filepath: str) -> None:
        ...
        with open(filepath, "w+", encoding=Charset.UTF_8.val) as f_configs:
            result = self._prompt(self.HHS_SETMAN_DB_FILE)
