...
def melon_scanner(host, delay=1):
    ...
    port_list = []
    new_port_list = random.sample(new_port_list[:], len(new_port_list[:]))
    for port in new_port_list:
        print(CYAN + f"checking port: {port}/tcp")
        my_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        my_socket.settimeout(15)
        try:
            my_socket.connect((new_host, int(port)))
            my_socket.close()
            print(CYAN + f"found port: {port}/tcp")
    ...
    new_port_list = random.sample(new_port_list[:], len(new_port_list[:]))
    for port in new_port_list:
        print(CYAN + f"checking port: {port}/udp")
        my_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        my_socket.settimeout(15)
        try:
            my_socket.sendto(b"",(new_host, int(port)))
            simple_response = my_socket.recvfrom(1)
            port_list.append(port)
            print(CYAN + f"found port: {port}/udp")
