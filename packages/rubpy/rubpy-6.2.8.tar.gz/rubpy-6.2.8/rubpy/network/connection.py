...
class Connection:
    ...
    async def download_chunk(self, session, server_url, headers, start_index, end_index):
        headers['start-index'] = str(start_index)
        headers['end_index'] = str(end_index - 1)
    ...
    async def download(self, dc_id: int, file_id: int, access_hash: str, size: int, chunk=131072, callback=None):
        ...
        async with aiohttp.ClientSession() as session:
            while start_index < size:
                ...
                chunk = await self.download_chunk(session, url, headers, start_index, end_index)
                if chunk:
                    chunks += chunk

                if callback:
                    await callback(size, len(chunks))
