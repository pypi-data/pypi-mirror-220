    ...
    for t, tests in dbt_tables.items():
        ...
        for test in tests:
            ...
            if bigeye_table.metric_time_column:
                ...
                # Create metrics if not outputing to bigconfig
                if not to_bigconfig:
                    log.info(f"Creating metric {test.metric_name} for table {source.name}.{schema_name}.{t}")
                    client.upsert_metric(metric_configuration=mc)

    if to_bigconfig:
        sbc = BigConfig(type='BIGCONFIG_FILE').create_bigconfig_from_metrics(
            tag_deployments=tag_deployments,
            collection_name='DBT Tests',
            collection_description='Collection for tracking dbt test results.')
        sbc.save(output_path=output_path,default_file_name='dbt_tests.bigconfig')