    ...
    if "PayloadType" in task_response.keys():
        if task_response["PayloadType"] == "Image":
            ...
            if "Legend" in task_response.keys() and image.height > 0 and image.width > 0:
                ...
                # If the first pixel is light, then the background of and we do not need to invert the colors
                #dark_theme = not image.getpixel((0, 0)) == LegendBuilder.get_legend_bg_color(dark_theme=False)
                ...
                #print("Is Dark Theme: ", dark_theme)
