...
ask_parser.add_argument("--threshold", default=0.2, type=float)
        ...
        if len(result["results"]):
            ...
            if args.gpt:
                ...
                for ans in result["results"]:
                    text += f"Answer: {ans['answer']}"
                    text += f"Context: {ans['context']}"
                prog = postprocess_program(query=text)
                result["answer"] = prog()
            elif args.opt:
                text = ""
                for ans in result["results"]:
                ...
                generator = pipeline(task='text-generation', model="facebook/opt-125m",
                                     max_new_tokens=200)
                res = generator(text, max_length=200)[0]
                result["answer"] = textwrap.fill(res["generated_text"])
            else:
                text = ""
                for ans in result["results"]:
                    text += f"{ans['context']}"
                ...
                result["answer"] = textwrap.fill(summary[0]["summary_text"])
