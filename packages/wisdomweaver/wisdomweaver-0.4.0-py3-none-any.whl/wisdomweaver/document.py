...
class Document:
    ...
    def load_sphinx_xml(self, xml_file):
        tree = etree.parse(str(xml_file))

        paragraphs = []
        for p in tree.xpath("//paragraph"):
            text = textwrap.dedent(str(p.text))
    ...
    def load_docbook(self, xml_file):
        tree = etree.parse(str(xml_file))

        ns = {"db": "http://docbook.org/ns/docbook"}
        paragraphs = [p.text for p in tree.xpath("//db:simpara", namespaces=ns)]
