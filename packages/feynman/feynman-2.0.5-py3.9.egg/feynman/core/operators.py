...
class Operator(Drawable):
    ...
    shape:
        'empty'
            Do not draw any shape. Other lines may still exist between
            vertices. May have text.
        'polygon'
            Only when N>2. A polygonal patch with cornes on the vertices.
        'ellipse'
            Only when N=2. An ellipse whose extrema land on vertices.
        'bubble'
            Only when N=4.
            Lines connecting v1 with v2 and v3 with v4,
            with an elliptic patch on top of those lines.
    ...
    **kwargs:
    ...
    def __init__(self, vertices, **kwargs):
        ...
        elif shape not in ('empty', 'polygon', 'ellipse', 'bubble'):
            raise Exception(
                "Unkownshape shape '{}'.format(shape)" +
                "Possible shapes are 'empty', 'polygon', 'ellipse', 'bubble'.")
