...
@click.pass_obj
def step_create(ctx: util.CliContext, stage_id: int, file: str, inventory_files: list) -> None:
    """
    ...
    response = api.post_request(ctx.api_url, api.STEP_CREATE, files=files, data={'stage_id': stage_id})

    util.echo_msg(response, 'Step successfully created!', ctx.debug)
...
@click.option('-i', '--inventory-file', 'inventory_files', type=click.Path(exists=True), multiple=True,
              help="Inventory file used to fill the template. Can be used multiple times.")
@click.pass_obj
def step_validate(ctx: util.CliContext, file: str, inventory_files: list) -> None:
    """
    ...
    response = api.post_request(ctx.api_url, api.STEP_VALIDATE, files=files)
...
@click.pass_obj
def step_execute(ctx: util.CliContext, step_id: int, stage_execution_id: int) -> None:
    """
    ...
    response = api.post_request(ctx.api_url, api.STEP_EXECUTE, step_id, custom_dict=arguments)

    util.echo_msg(response, 'Step successfully executed!', ctx.debug)
...
def step_execution_report(ctx: util.CliContext, execution_id: int, file: str, less: bool, localize: bool) -> None:
    ...
    util.get_yaml(response, file, f'report-step-execution_{execution_id}', less, less, localize, ctx.debug)
