...
class EvaluateModel():
    ...
    def make_evaluation(self, df_real: pd.DataFrame, df_pred: pd.DataFrame, id_pred: str, verbose: bool =True) -> pd.DataFrame:
        ...
        if verbose:
            logger.info(f'Evaluating results for {id_pred}')
        ...
        if len(inter) == 0:
            logger.warning(f'Length of intersection between Predictions and Ground Truth Datasets for {id_pred} is Null. Probably one of the two Datasets has not available data (all NaN).')
