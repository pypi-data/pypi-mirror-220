    ...
    if ext is not None:
        ...
        if ':' in pos_file_path:
            ...
            if not re.search(fr"^{Util.SOLIDITY_CONTRACT_NAME_RE}$", contract):
                raise Util.CertoraUserInputError(
                    f"contract name {contract} should contain only alphanumeric characters, "
                    f"underscores or dollar signs")
        ...
        if not re.search(fr"^{Util.SOLIDITY_CONTRACT_NAME_RE}$", base_name):
            raise Util.CertoraUserInputError(
                f"file name {file} can contain only alphanumeric characters, underscores or dollar signs")
        return file
