...
from location.src.get_country_name import country
...
class Importer:
    ...
    def insert_new_entity(self, entity_type_name):
        ...
        if not entity_type_id:
            ...
            logger_local.end("End Inserted Entity %s ." % entity_type_name)
            db.commit()
        else:
    ...
    def insert_new_source(self):
        logger_local.start("Start insert_new_source()")
        database_conn = database()
        db = database_conn.connect_to_database()
        cursor = db.cursor()
        ...
        if not source_id:
            ...
            db.commit()
            logger_local.end("end insert_new_source %s."%self.source_name)
        else:
            logger_local.end("Source %s already exist." % self.source_name)
        db.close()
    ...
    def insert_record_source(self, location, entity_type_name, entity_id, url):
        ...
        db.commit()
        db.close()
        logger_local.end("Function insert_record_source() ended")
