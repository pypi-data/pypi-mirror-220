...
class Import:
    ...
    def to_html(self) -> str:
        ...
        if rel == "stylesheet":
            ...
            return f"<script type=\"{self.kind}\" src=\"{self.path}\"></script>"
...
class Tran:
    ...
    def to_html(self) -> str:
        ...
        return self.doctype.to_html() + \
            "<head xmlns=\"\">"+"".join(tuple(i.to_html() for i in self.data["meta"])) + \
            "</head>\n\n" + \
            self.cache.replace("\n", "\n    ").strip()
    ...
    def walk_subtree(self, node: Optional[Node | list] = None):
        ...
        if type(node) == list:
            ...
            if node.kind == "doctype":
                ...
                return Import(node.data["type"], node.data["semantic"], node.data["path"]).to_html()
