...
from . import value_decrease_indication_v1  # noqa
...
@versioning.register(
    version="1.0", changelog="Rename function from negative_running_hours_check to value_decrease_check"
)
@check_types
def value_decrease_check(x: pd.Series, threshold: float = 0.0) -> pd.Series:
    """Decrease in time series values.

    Identify decrease in values of a time series where the values shouldn't be decreasing over time. One example would
    ...
    all time series where the decrease in value is a sign of bad data quality.
