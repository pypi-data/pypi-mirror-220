...
class BACDevice(object):
    ...
    def getProperty(self, name, update=False):
        try:
            ...
            # value=self._device.bacnet_properties[name].
            value=self._device._bacnet_properties(update)[name]
            if update or value is None:
                # print("REFRESH", name)
                value=self._device._bacnet_properties(True)[name]
            return value
        except:
            pass
    ...
    def poll(self, delay=15):
        ...
        self.pollStop()
        self._device.poll(delay=delay)

    def pollStop(self):
        """Unregister the device polling task"""
        self._device.poll(command='stop')

    def isPolled(self):
        if self._device.properties.pollDelay:
            return True
        return False
