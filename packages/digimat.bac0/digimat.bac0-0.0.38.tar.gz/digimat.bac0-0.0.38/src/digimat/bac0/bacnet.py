...
class BACPointMultiState(BACPoint):
    ...
    def state(self):
        try:
            ...
            return '%s' % (self.properties.units_state[index-1])
        except:
            pass
        return self.value
...
class BACPoints(object):
    ...
    def dump(self, keys=None):
        ...
        if points:
            t=PrettyTable()
            t.field_names=['#', 'name', 'description', 'type', 'address', 'value', 'state', 'unit', 'COV', 'OoS', 'PRI']
            t.align['name']='l'
            # t.align['class']='l'
