...
def import_class_at_runtime(path: str):
    ...
    try:
        workload = getattr(
            pgworkload_odbc.utils.builtin_workloads, path.lower().capitalize()
        )
...
def run_transaction(conn: pyodbc.Connection, op, max_retries=3):
    """
    ...
    for retry in range(1, max_retries + 1):
        try:
            ...
            if e.args[0] == '40001':
                ...
                logger.debug(f"pyodbc.SerializationFailure:: {e}")
                conn.rollback()
...
def get_dbms(dburl: str):
    ...
    """
    try:
        with pyodbc.connect(dburl, autocommit=True) as conn:
