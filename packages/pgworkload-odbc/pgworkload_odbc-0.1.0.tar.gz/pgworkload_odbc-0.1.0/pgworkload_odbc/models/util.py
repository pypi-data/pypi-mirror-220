...
def util_csv(
    ...
    if not output:
        output_dir = pgworkload_odbc.utils.util.get_based_name_dir(input)
    else:
        output_dir = output
    ...
    pgworkload_odbc.utils.simplefaker.SimpleFaker(csv_max_rows=csv_max_rows).generate(
    ...
    if not http_server_hostname:
        http_server_hostname = pgworkload_odbc.utils.util.get_hostname()
        logger.debug(f"Hostname identified as: '{http_server_hostname}'")

    stmt = pgworkload_odbc.utils.util.get_import_stmt(
...
def util_yaml(input: str, output: str):
    ...
    if not output:
        output = pgworkload_odbc.utils.util.get_based_name_dir(input) + ".yaml"
    ...
    with open(output, "w") as f:
        f.write(pgworkload_odbc.utils.util.ddl_to_yaml(ddl))
