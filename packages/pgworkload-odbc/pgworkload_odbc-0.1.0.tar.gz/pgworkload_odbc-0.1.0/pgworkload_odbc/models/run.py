...
def run(
    ...
    stats = pgworkload_odbc.utils.util.Stats(frequency, prom_port)
    ...
    threads_per_proc = pgworkload_odbc.utils.util.get_threads_per_proc(procs, conc)
    ...
    try:
        ...
        while True:
            ...
            if c >= concurrency:
                if isinstance(tup, pyodbc.Error):
                    logger.error(tup)
                    logger.error(
...
def worker(
    ...
    while True:
        ...
        try:
            ...
            pass
        try:
            with pyodbc.connect(dburl, autocommit=autocommit) as conn:
                ...
                while True:
                    ...
                    for txn in w.run():
                        start = time.time()
                        pgworkload_odbc.utils.util.run_transaction(
