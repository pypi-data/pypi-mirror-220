...
class StackTemplate():
    ...
    @property
    def app_name(self):
        """The Application name or None. Only Application resources have a Resource Group name,
        e.g. BackupVault Resource does not.
        """
        app = get_parent_by_interface(self.stack.resource, schemas.IApplication)
        if app != None:
            return app.name
        return None
        ...
        while True:
            ...
            while True:
                ...
                if next_ref_idx != -1:
                    ...
                    sub_value = self.paco_ctx.get_ref(sub_ref, resolve_from_outputs=False)
                    if sub_value == None:
                        sub_value = self.paco_ctx.get_ref(sub_ref, resolve_from_outputs=True)
                    if sub_value == None:
        ...
        for list_item in value:
            if is_ref(list_item):
                ...
                if isinstance(stack, Stack) == False:
                    raise PacoException(PacoErrorCode.Unknown, message=f"Reference must resolve to a stack:\n{list_item}")
