...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--disable_submit_results", action='store_true', help="Disable submitting the results")
...
class WebUtils:
    ...
    def put_response_with_timeout(self, url: str, data: Union[str, Dict[str, Any]]) -> Optional[requests.Response]:
        """
...
def submit(args: argparse.Namespace) -> None:
    ...
    if not args.sync:
        ...
        mutation_id, anonymous_key, presigned_url = send_get_request(web_utils, type="async")
        upload_file_to_cloud_storage(web_utils, presigned_url, collect_data)
        generate_report_url(web_utils.ui_url, mutation_id, anonymous_key)
    else:
...
def collect(args: argparse.Namespace) -> bool:
    ...
    if not args.disable_submit_results:
        submit_to_ui(ui_elements, web_utils, args.ui_out)
    return True
...
def send_get_request(web_utils: WebUtils, type: str = "") -> Tuple[str, str, str]:
    ...
    return resp_obj[Constants.ID], resp_obj[Constants.ANONYMOUS_KEY], resp_obj[Constants.PRE_SIGNED_URL]
...
def upload_file_to_cloud_storage(web_utils: WebUtils, presigned_url: str,
    ...
    if put_resp is None or put_resp.status_code != 200:
        print("Failed to upload the file")
        logging.debug("Failed to submit to presigned URL")
        logging.debug("Didn't get any response" if put_resp is None else
                      f"The status code was {put_resp.status_code}")
        sys.exit(1)
...
def submit_to_ui(elems: List[Any], web_utils: WebUtils, ui_out: Optional[str]) -> None:
    mutation_id, anonymous_key, presigned_url = send_get_request(web_utils)

    to_json = [dataclasses.asdict(e) for e in elems]
    upload_file_to_cloud_storage(web_utils, presigned_url, json.dumps(to_json))
