...
class MultiExperimentHarness():
    ...
    def _get_all_args(self):
        ...
        for experiment in self.experiments:
            for name, arg in experiment.all_args.items():
                args[name].extend(arg)
        print(args)
        return args
    ...
    def visualize(self, colname: str = None) -> None:
        ...
        data = {
            'prompt': [str(combo['prompt']) for combo in argument_combos],
        ...
        for k, args in self._get_all_args().items():
            if len(args) > 1:
                data[k] = [combo[k] for combo in argument_combos]
