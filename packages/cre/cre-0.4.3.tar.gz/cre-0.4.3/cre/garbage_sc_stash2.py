    ...
    for func in depth_policy:
        ...
        if(isinstance(func, tuple)):
            ...
            for inds in arg_inds:
                ...
                # print(v)
                rec = apply_one(self, func, self.curr_infer_depth, inds)
...
apply_one_impl_cache = {}
def get_apply_one_impl(func):
    return_type, arg_types = func.return_type, tuple(func.arg_types)
    tup = (return_type, arg_types)
    if(tup not in apply_one_impl_cache):
        lst_types = tuple([ListType(arg_type) for arg_type in arg_types])
        context = cre_context()
        arg_t_ids = tuple([u2(context.get_t_id(_type=t)) for t in arg_types])
        ret_d_typ = DictType(return_type,Tuple((i8,i8,i8)))  
        ret_type_t_id = u2(context.get_t_id(_type=return_type))
        func_type = CREFuncTypeClass(return_type, arg_types)

        @njit
        def call_func_for_inds(planner, func, depth, inds):
            arg_ptrs = np.empty(len(arg_types),dtype=np.int64)
            i = 0
            for lst_typ in literal_unroll(lst_types):
                iter_base = _list_base_from_ptr(planner.flat_vals_ptr_dict[(u2(arg_t_ids[i]),depth)])
                size = _listtype_sizeof_item(lst_typ)
                arg_ptrs[i] = iter_base + inds[i] * size
                i += 1
        ...
        @njit(types.optional(SC_RecordType)(GenericSetChainingPlannerType, CREFuncType, i8, i8[::1]), cache=True)
        def apply_one_impl(planner, func, curr_infer_depth, inds):
            v = call_func_for_inds(planner, cast(func, func_type), curr_infer_depth, inds)
            if(v is None):
                return
        ...
        ep = apply_one_impl.overloads[(GenericSetChainingPlannerType, CREFuncType, i8, i8[::1])].entry_point
        apply_one_impl_cache[tup] = ep
    return apply_one_impl_cache[tup]

from cre.default_funcs import Add

# Precompile for these signatures
get_apply_one_impl(Add(f8,f8))
# get_apply_one_impl(Concat(unicode_type,unicode_type))

def apply_one(planner, func, curr_infer_depth, inds):
    impl = get_apply_one_impl(func)
    return impl(planner, func, curr_infer_depth, inds)
# @generated_jit(cache=True, nopython=True)
