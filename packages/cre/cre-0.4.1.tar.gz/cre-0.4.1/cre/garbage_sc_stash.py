...
def get_ensure_ptr_dicts_impl(typ):
    lt = ListType(typ)
    vt = DictType(typ, i8_3x_tuple)
    ivt = DictType(i8, typ)
    typ_name = str(typ)
    typ_t_id = u2(cre_context().get_t_id(typ))

    @njit
    def ensure_ptr_dicts_impl(planner):
    ...
    return ensure_ptr_dicts_impl

# -------------
# : planner_declare_val

def get_planner_declare_val_impl(val_typ, func_or_var_type):
    '''Declares a primative value or fact (but not its visible attributes)
        into the 0th depth of a planner instance.'''
    ensure_ptr_dicts = get_ensure_ptr_dicts_impl(val_typ)
    @njit
    def planner_declare_val_impl(planner, val, func_or_var):
        # Ensure that various data structures exist for this val_type
        flat_vals, val_map, inv_val_map, declare_records = \
            ensure_ptr_dicts(planner)
        ...
        for i, _val in enumerate(flat_vals):
            if(_val == val):
                return i
        return -1
    return planner_declare_val_impl

def planner_declare_val():
    pass

@overload(planner_declare_val)
def overload_planner_declare_val(planner, val, func_or_var):
    impl = get_planner_declare_val_impl(val, func_or_var)
    return  impl
...
# -------------
# : planner_declare_conversion

planner_declare_conversion_cache = {}
def get_planner_declare_conversion_impl(val_typ, func_or_var_type):

    tup = (val_type, func_or_var_type)
    if(tup not in planner_declare_conversion_cache):
        ensure_ptr_dicts = get_ensure_ptr_dicts_impl(val_typ)
        
        @njit(i8(GenericSetChainingPlannerType, val_typ, func_or_var_type, types.optional(i8)), cache=True)
        def planner_declare_conversion_impl(planner, val, func_or_var, source_ind=None):
            # Ensure that various data structures exist for this val_type
            flat_vals, val_map, inv_val_map, declare_records = \
        ...
        planner_declare_conversion_cache[tup] = planner_declare_conversion_impl
    return planner_declare_conversion_cache[tup]


# -------------
# : planner_declare_fact_conversions

def get_planner_declare_fact_conversions_impl(fact_type):
    attr_spec = fact_type.clean_spec[attr.literal_value]

    assert 'conversions' in attr_spec
    ...
    @njit
    def planner_declare_fact_conversions_impl(planner, val, fact_type, attr, source_ind):
    ...
    return planner_declare_fact_conversions_impl
...
def planner_declare_overload(planner, val, var=None):
    ...
    if(isinstance(val_typ, Fact)):
        ...
        if(len(semantic_visible_attrs) > 0):
            ...
            planner_declare_fact_conversions = get_planner_declare_fact_conversions_impl(val_typ)
            ...
            def impl(planner, val, var=None):
                ...
                for tup in literal_unroll(semantic_visible_attrs):
                    ...
                    planner_declare_fact_conversions(planner, attr_val, fact_type, attr, ind)
        else:
...
from cre.default_funcs import Add, Concat
