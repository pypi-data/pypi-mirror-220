...
from .database import Database, DatabaseQuery, MicroserviceDatabaseManager
...
class Mongo(Database):
    ...
    async def insert_admin(self, obj: root.DesktopObject | root.DesktopObjectDict, collection: str, default_content: Optional[IO] = None):
        """
        Insert a desktop object into the database, with its content if provided, and with no permission checking.
...
class MongoContext(DatabaseQuery[Mongo]): # Go into db package?
    """
    ...
    async def connection(self) -> Mongo:
        return self.request.app[HEA_DB]
