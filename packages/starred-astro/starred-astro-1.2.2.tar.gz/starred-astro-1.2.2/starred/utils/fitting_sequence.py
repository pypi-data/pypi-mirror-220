...
def run_2_steps_PSF_reconstruction(data, model, parameters, sigma_2, lambda_scales = 1., lambda_hf =1, lambda_positivity=0.,
                                   kwargs_optim1=None, kwargs_optim2=None, method_noise = 'MC',
                                   method1='l-bfgs-b', method2='adabelief', regularize_full_psf = False,
                                   masks=None):
    """
    ...
    kwargs_init, kwargs_fixed, kwargs_up, kwargs_down = deepcopy(parameters._kwargs_init), deepcopy(parameters._kwargs_fixed), \
    ...
    best_fit, logL_best_fit, extra_fields, runtime = optim.minimize(restart_from_init = True, **kwargs_optim1)
    ...
    W = propagate_noise(model, np.sqrt(sigma_2), kwargs_partial, wavelet_type_list=['starlet'], method=method_noise,
                        num_samples=500, seed=1, likelihood_type='chi2', verbose=False, upsampling_factor=model.upsampling_factor)[0]

    # Release backgound, fix the moffat
    if model.elliptical_moffat:
        kwargs_moffat_fixed = {'fwhm_x': kwargs_partial['kwargs_moffat']['fwhm_x'],'fwhm_y': kwargs_partial['kwargs_moffat']['fwhm_y'],
                               'phi': kwargs_partial['kwargs_moffat']['phi'],
                              'beta': kwargs_partial['kwargs_moffat']['beta'], 'C': kwargs_partial['kwargs_moffat']['C']}
    else:
...
def multi_step_deconvolution(data, model, parameters, sigma_2, s, subsampling_factor,
                              ...
                              lambda_scales = 1., lambda_hf =1, lambda_positivity_bkg = 100, lambda_positivity_ps=100,
    ...
    for i, steps in enumerate(fitting_sequence):
        ...
        parameters = ParametersDeconv(model, kwargs_partial_list[i], kwargs_fixed, kwargs_up=kwargs_up, kwargs_down=kwargs_down)
        ...
        loss = deconvLoss(data, model, parameters, sigma_2, regularization_terms='l1_starlet',
                          ...
                          regularization_strength_positivity=lambda_positivity_bkg,
        ...
        # compute noise level in starlet space, only after the first step
        if i == 0:
            W = propagate_noise(model, np.sqrt(sigma_2), kwargs_partial_steps, wavelet_type_list=['starlet'], method=noise_propagation,
                                num_samples=500, seed=1, likelihood_type='chi2', verbose=False, upsampling_factor=subsampling_factor)[0]
