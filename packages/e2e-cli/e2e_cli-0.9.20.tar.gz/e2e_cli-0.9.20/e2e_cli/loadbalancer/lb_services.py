...
from e2e_cli.core.constants import CONFIRMATION_MSG, YES
...
class LBServices:
    ...
    def assign_nodes(self, nodes_assigned_to_lb, api_key, auth_token):
        ...
        while want_to_go_further.lower() == YES:
            node_choice = self.choice_nodes(
                nodes_assigned_to_lb, node_list_dict)
            if node_choice == '/':
                break
            print("Node name: ", node_list_dict[node_choice]["name"])
            print("Node ip: ",
                  node_list_dict[node_choice]["private_ip_address"])
            node_port = input("Enter the Node Port: ")
            while (not Checks.is_int(node_port)):
                node_port = input("Enter the Node Port: ")
            nodes_assigned_to_lb.append(
    ...
    def classic_lb_creation(self, api_key, auth_token, nodes_assigned_to_lb, plan_assigned_to_lb):
        ...
        while (not lb_balancing_property_choice in lb_balancing_property):
            lb_balancing_property_choice = input(
                "Please select from given choices only: ")
        balancing_property_assigned_to_lb = lb_balancing_property[lb_balancing_property_choice]

        nodes_assigned_to_lb = self.assign_nodes(
            nodes_assigned_to_lb, api_key, auth_token)
        payload = json.dumps({
            "acl_list": [],
            "acl_map": [],
            "backends": [
    ...
    def acl_condition_http(self):
        ...
        table.add_row(["3", Methods.GET])
        table.add_row(["4", Methods.POST])
        table.add_row(["5", Methods.PUT])
        table.add_row(["6", Methods.DELETE])
        ...
        if acl_condition_choice == "1":
            ...
            acl_conditions.append(Methods.GET)
        elif acl_condition_choice == "4":
            acl_conditions.append(Methods.POST)
        elif acl_condition_choice == "5":
            acl_conditions.append(Methods.PUT)
        elif acl_condition_choice == "6":
            acl_conditions.append(Methods.DELETE)
        elif acl_condition_choice == "7":
    ...
    def acl_rules(self, acl_rules_assigned):
        ...
        while add_more_rules.lower() == YES:
            rule_count += 1
            acl_rules_assigned.append(self.set_acl_rules(rule_count))
            add_more_rules = input("Want to add more rules(y/n): ")
    ...
    def set_backends(self, backend_server_list, backends_assigned_to_rule):
        ...
        for backend_instance in backend_server_list:
            if backend_instance["backend_name"] in backends_assigned_to_rule:
                ...
                table.add_row([backend_instance["backend_id"],
    ...
    def select_rules(self, acl_rules):
        ...
        for rule_instance in rules_object_list:
            table.add_row([rule_id, rule_instance["rule"],
                          rule_instance["rule_type"]])
            rule_id += 1
        print(table)
        rule_choice = input(": ")
    ...
    def acl_maps(self, acl_maps_list, acl_rules, backends):
        ...
        backend_choice = self.set_backends(
            backend_server_list, rules_to_backends[your_rule_choice])
        ...
        rules_to_backends[your_rule_choice].append(
            backend_server_list[int(backend_choice) - 1]["backend_name"])
        ...
        while add_more_choice.lower() == YES:
            your_rule_choice = self.select_rules(acl_rules)
            backend_server_list = []
            for backend_index in range(0, len(backends)):
                backend_server_list.append(
    ...
    def advance_lb_creation(self, api_key, auth_token, acl_rules_assigned_to_lb, acl_maps_to_backends,
        ...
        if not backends_assigned_to_lb:
            backends_assigned_to_lb = self.backends(
                api_key, auth_token, backends_assigned_to_lb, [])
            add_more_backend = input("Want to add more backend(y/n): ")
            while add_more_backend.lower() == YES:
                backends_assigned_to_lb = self.backends(
                    api_key, auth_token, backends_assigned_to_lb, [])
                add_more_backend = input("Want to add more backend(y/n): ")
        else:
        ...
        acl_maps_to_backends = self.acl_maps(
            acl_maps_to_backends, acl_rules_assigned_to_lb, backends_assigned_to_lb)
        payloads = {
    ...
    def all_lb(self):
        ...
        if api_key_credentials_object["message"] == "Invalid alias provided":
            ...
            payload = {}
            method = Methods.GET
            response_object = Request(request_method=method, url=url,
                                      auth_token=api_key_credentials_object["api_credentials"]["api_auth_token"],
                                      payload=payload)
            return {"lb_api_response": response_object.make_api_call(),
    ...
    def get_lb_by_id(self, lb_id):
        if (Checks.is_int(lb_id)):
            ...
            found_dict = next(
                (d for d in lb_list if d['id'] == int(lb_id)), None)
        else:
            found_dict = None
        return found_dict
        ...
        if api_key_credentials_object["message"] == "Invalid alias provided":
            ...
            if int(choice_of_lb) == 1:
                ...
                url = "api/v1/appliances/load-balancers/" \
                    "?apikey=" + \
                    api_key_credentials_object["api_credentials"]["api_key"]
                response_object = Request(request_method=Methods.POST, url=url,
                                          auth_token=api_key_credentials_object[
                                              "api_credentials"]["api_auth_token"],
                                          payload=payload)
                return {"lb_api_response": response_object.make_api_call(),
                        "message": api_key_credentials_object["message"]}
    ...
    def delete_lb_service(self):
        ...
        if choice.upper() == YES:
            ...
            if api_key_credentials_object["message"] == "Invalid alias provided":
                ...
                payload = {}
                response_object = Request(request_method=Methods.DELETE, url=url,
                                          auth_token=api_key_credentials_object[
                                              "api_credentials"]["api_auth_token"],
                                          payload=payload)
                return {"lb_api_response": response_object.make_api_call(),
    ...
    def select_backend(self, backend_assigned):
        ...
        for backend_index in range(0, len(backend_assigned)):
            table.add_row(
                [str(backend_index + 1), "backend-server-" + str(backend_index + 1)])
        print(table)
        backend_choice = input(": ")
        return backend_choice
    ...
    def select_node(self, nodes_assigned):
        ...
        for node_index in range(0, len(nodes_assigned)):
            table.add_row(
                [str(node_index + 1), nodes_assigned[node_index]["backend_name"]])
        print(table)
        node_choice = input(": ")
        return node_choice
    ...
    def change_port_node(self, appliances):
        ...
        while change_choice.lower() == YES:
            backend_assigned = appliances_assigned["context"]["backends"]
            backend_choice = self.select_backend(backend_assigned)
            nodes_assigned = backend_assigned[int(
                backend_choice) - 1]["servers"]
            print(" ")
            node_choice = self.select_node(nodes_assigned)
            node_port = input(" Node Port: ")
            nodes_assigned[int(node_choice) - 1]["backend_port"] = node_port
    ...
    def edit_lb(self):
        ...
        if api_key_credentials_object["message"] == "Invalid alias provided":
            ...
            if edit_choice == str(1):
                ...
                url = "api/v1/appliances/load-balancers/" + \
                    lb_id + "/actions/?apikey=" + \
                    api_key_credentials_object["api_credentials"]["api_key"]
                response_obj = Request(request_method=Methods.PUT, url=url,
                                       auth_token=api_key_credentials_object["api_credentials"]["api_auth_token"],
                                       payload=json_payload)
                return {"lb_api_response": response_obj.make_api_call(),
                        "message": api_key_credentials_object["message"]}
            elif edit_choice == str(2):
                json_payload = self.change_port_node(
                    all_lb_in_list["appliance_instance"])
                url = "api/v1/appliances/load-balancers/" + \
                    lb_id + "/?apikey=" + \
                    api_key_credentials_object["api_credentials"]["api_key"]
                response_obj = Request(request_method=Methods.PUT, url=url,
                ...
                json_payload = self.add_new_node(
                    all_lb_in_list["appliance_instance"], api_key_credentials_object)
                url = "api/v1/appliances/load-balancers/" + \
                    lb_id + "/?apikey=" + \
                    api_key_credentials_object["api_credentials"]["api_key"]
                response_obj = Request(request_method=Methods.PUT, url=url,
