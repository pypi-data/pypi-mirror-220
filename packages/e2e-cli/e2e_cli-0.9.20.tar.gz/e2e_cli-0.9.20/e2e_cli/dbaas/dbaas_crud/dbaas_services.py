...
from e2e_cli.core.constants import YES
...
class DBaaSServices:
    ...
    def all_dbaas(self):
        ...
        if api_key_credentials_object["message"] == "Invalid alias provided":
            ...
            response = json.loads(Request(
                request_method=Methods.GET, url=url,  auth_token=auth_token, payload=payload).make_api_call().content)
    ...
    def select_a_software(self, api_key, api_auth_token):
        ...
        response = Request(request_method=Methods.GET, url=url,
                           auth_token=auth_token, payload=payload).make_api_call()
        ...
        table = PrettyTable(
            ["Software ID", "Software Name", "Software Version"])
        for software_instance in software_engines:
            table.add_row(
                [software_instance["id"], software_instance["name"], software_instance["version"]])
        print(table)
        software_id = input(": ")
    ...
    def select_a_template(self, software_id, api_key, api_auth_token):
        ...
        for template_instance in template_engines:
            table.add_row(
                [templates_id, template_instance["name"], template_instance["price"]])
            templates_id += 1
        print(table)
    ...
    def add_dbaas(self):
        ...
        while (name_assigned_to_database == "" or " " in name_assigned_to_database):
            name_assigned_to_database = input(
                "Re-enter, name can't have spaces :")
        username_assigned_to_database = input(
            "Assign UserName to your database(important): ")
        while (username_assigned_to_database == "" or " " in username_assigned_to_database):
            username_assigned_to_database = input(
                "Re-enter, username can't have spaces :")
        password_assigned_to_database = input(
            "Assign Password to your database(important): ")
        ...
        if api_key_credentials_object["message"] == "Invalid alias provided":
            ...
            if software_id == '/':
                ...
                auth_token = api_key_credentials_object["api_credentials"]["api_auth_token"]
                response = Request(
                    request_method=Methods.POST, url=url, auth_token=auth_token, payload=payload).make_api_call()
    ...
    def delete_dbaas(self):
        ...
        if choice.upper() == YES:
            ...
            if api_key_credentials_object["message"] == "Invalid alias provided":
                ...
                auth_token = api_key_credentials_object["api_credentials"]["api_auth_token"]
                response = Request(
                    request_method=Methods.DELETE, url=url, auth_token=auth_token, payload=payload).make_api_call()
