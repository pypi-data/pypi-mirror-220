...
from tdm.datamodel.facts.concept import MissedConceptValue
...
from .directive import CreateConceptDirectiveModel, DirectiveModel, DirectiveType
...
class TalismanDocumentModel(BaseModel):
    ...
    def to_doc(
        ...
        id2directive = {d.id: d for d in type2directives[DirectiveType.CREATE_CONCEPT]}
        ...
        for old_concept_fact in old_facts.get(FactType.CONCEPT, []):
            ...
            if old_concept_fact.value in id2directive:
                directive: CreateConceptDirectiveModel = id2directive[old_concept_fact.value]
                concept_fact = replace(concept_fact, value=MissedConceptValue(directive.filters, directive.name))
