...
from tdm.helper import cache_result, generics_mapping, register_in_module, unfold_union
from .serializers import AbstractElementModel, AbstractElementSerializer, CompositeElementSerializer, get_serializer
...
def create_model_for_type(type_: Type[_Element], label: Optional[str] = None) -> Type[ElementModel[_Element]]:
    ...
    for field in fields(type_):
        ...
        for t in map(lambda ut: type_vars.get(ut, ut), union_types):
            typing_type, serializer = get_serializer(t)
            field_types.append(typing_type)
            serializers.append(serializer)
        field_type = Union[tuple(field_types)]
        serializer = CompositeElementSerializer.build(serializers)
