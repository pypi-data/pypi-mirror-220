...
@Pipe
def xThreadPoolExecutor(iterable, func, max_workers=5, thread_name_prefix=''):
    """
    with ThreadPoolExecutor(max_workers) as pool:
        pool.map(func, iterable)
    """
    if max_workers > 1:
        with ThreadPoolExecutor(max_workers, thread_name_prefix=thread_name_prefix) as pool:
...
@Pipe
def xProcessPoolExecutor(iterable, func, max_workers=5, method=None):
    """
    ...
    mp_context = None
    if method:
        mp_context = multiprocessing.get_context(method=method)

    if max_workers > 1:
        with ProcessPoolExecutor(max_workers, mp_context=mp_context) as pool:
            return pool.map(func, iterable)
    else:
        return map(func, iterable)
