...
class Harvester(object):
    def __init__(
        ...
        self.logger = logging.getLogger(str(self.logger_name) + str(self.logger_id))
        ...
        httpHandler = logging.handlers.HTTPHandler(host=self.logger_handler_host, url='/api/logger_post/', method='POST')
            ...
            if datetime_filter[0] is not None:
                try:
                    ...
                    if not isinstance(datetime_after, datetime):
                        self.logger.error("'datetime_after' parameter have to be a datatime object")
            ...
            if datetime_filter[1] is not None:
                try:
                    ...
                    if not isinstance(datetime_before, datetime):
                        self.logger.error("'datetime_before' parameter have to be a datatime object")
                    else:
        ...
        if stac is not False:
            ...
            if len(urls) != 0:
                ...
                self.logger.warning("Webservices are not activated for this catalog")
                # print(
                                    ...
                                    for tags in root_nested.iter():
                                        ...
                                        if self.auto_lists != []:
                                            try:
                                                ...
                                                self.logger.warning("NcML service is not activated")
                                                try:
                                                    core.harvesting_core(
                                                        tags,
                                                        self.auto_lists[1],
                                                    ...
                                                    self.logger.warning("WMS service is not activated")
                                                    try:
                                                        core.harvesting_core(
                                                            tags,
                                                            self.auto_lists[2],
                                                        ...
                                                        self.logger.warning("ISO service is not activated")
                                                        continue
                                        else:
