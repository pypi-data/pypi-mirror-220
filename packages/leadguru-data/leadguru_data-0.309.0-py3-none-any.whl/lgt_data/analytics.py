...
def get_global_saved_leads_analytics(from_date: datetime = None, to_date: datetime = None):
    ...
    if from_date:
        pipeline.insert(0, {"$match": {"created_at": {"$gte": from_date}}})

    if to_date:
        pipeline.insert(0, {"$match": {"created_at": {"$lte": to_date}}})
...
def get_total_active_bots_global_analytics(from_date: datetime.datetime = None, to_date: datetime.datetime = None):
    ...
    pipeline = [
        {
            '$match': {
                'action': 'monitoring.start.workspace',
                'created_at': {
                    '$lt': from_date
                }
            }
        }]
    bots = list(client.lgt_admin.user_track_action.aggregate(pipeline))
    response = {}
    for date in active_bot_dates:
        count = sum(1 for bot in bots if bot["created_at"] <= date)
