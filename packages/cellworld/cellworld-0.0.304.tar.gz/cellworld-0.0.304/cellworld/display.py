...
class Display:
    ...
    def __process_click__(self, event):
        if self.on_cell_click:
            ...
            cell = self.world[cell_id]
            if cell.location.dist(location) < self.world.implementation.cell_transformation.size:
                self.on_cell_click(event.button, self.world[cell_id])

    def set_cell_clicked_event(self, callback):
        self.on_cell_click = callback
        self.fig.canvas.mpl_connect('button_press_event', self.self.__process_click__)
