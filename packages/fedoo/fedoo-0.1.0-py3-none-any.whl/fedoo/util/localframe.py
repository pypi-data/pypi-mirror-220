...
class LocalFrame(np.ndarray):
    ...
    def Rotate(self, angle, axis = 'Z'):
        ...
        if angle is not 0:           
            ...
            if axis.upper() == 'X': axis = (1,0,0)
            elif axis.upper() == 'Y': axis = (0,1,0)
            elif axis.upper() == 'Z': axis = (0,0,1)
            
            s = np.sin(angle) ; c = np.cos(angle) ; 
            x = axis[0] ; y = axis[1] ; z = axis[2]
            axis = np.array([axis])      
            
            RotMatrix = np.array([[ (1-c) * x**2 + c  , (1-c) * x*y - z*s, (1-c) * x*z + y*s], 
                                  [ (1-c) * x*y  + z*s, (1-c) * y**2 + c , (1-c) * y*z - x*s],
                                  [ (1-c) * x*z  - y*s, (1-c) * y*z + x*s, (1-c) * z**2 + c ]])
            
            if len(RotMatrix.shape) == 3: RotMatrix = np.transpose(RotMatrix,[2,1,0])
            elif len(RotMatrix.shape) == 2: RotMatrix = RotMatrix.T 
...
#        return H
    ...
    def __getitem__(self, index):
        ...
        if new.ndim == 3 and new.shape[1] in [2,3] and new.shape[2] in [2,3]: return new
...
def GenerateCylindricalLocalFrame(crd, axis=2, origin = [0,0,0], dim=3):
    ...
    localFrame[:, 0, plane] = crd[:,plane]/np.sqrt(crd[:,plane[0]]**2+crd[:,plane[1]]**2).reshape(-1,1) #er              
