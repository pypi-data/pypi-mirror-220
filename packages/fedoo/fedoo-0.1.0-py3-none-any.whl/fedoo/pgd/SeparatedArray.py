...
from numbers import Number #class de base qui permet de tester si un type est numérique
...
class SeparatedArray: #Fonction discrete sous forme séparée
    ...
    def __add__(self, A):  # somme        
        if isinstance(A, SeparatedArray):            
            if self.dim == A.dim and self.shape == A.shape:
                return SeparatedArray([ np.c_[self.data[dd],A.data[dd]] for dd in range(self.dim) ])
            else: raise NameError('Dimensions doesnt match')
        elif isinstance(A, Number) == 1: 
            if A == 0: return self
            else: return self+SeparatedArray(A,self.shape)
        else: return NotImplemented

    def __div__(self, a): #division (type numérique uniquement)
        if isinstance(a, Number) == 1: 
            return SeparatedArray([ np.c_[self.data[0]/a] ] + [ np.c_[self.data[dd]] for dd in range(1,self.dim) ])            
        else: return NotImplemented
    ...
    def __getitem__(self, key): 
        ...
        for ii, kk in enumerate(key):
            ...
            elif isinstance(kk, slice): ind.append(kk) ; dim.append(ii) #ind.append(range(self.shape[ii]).__getitem__(kk)); dim.append(ii)
            elif isinstance(kk, list) or (isinstance(kk,np.ndarray) and len(kk.shape) == 1): ind.append(kk) ; dim.append(ii)
            else: assert 0, "Only support numeric, slice, list or np.ndarray key"
                                    
        if len(dim) == 0: return np.sum(np.prod([self.data[dd][key[dd], :] for dd in range(self.dim)] , 0))
        elif len(dim) == 1: 
            if len(self.data) == 1:
                return np.sum(self.data[0],1) 
            else:
                return np.dot(self.data[dim[0]][ind[dim[0]],:], np.prod([self.data[dd][key[dd], :] for dd in list(range(dim[0]))+list(range(dim[0]+1,self.dim))] , 0) )
        elif len(dim) == 2:      
            temp = np.prod([self.data[dd][key[dd], :] for dd in list(range(dim[0]))+list(range(dim[0]+1,dim[1]))+list(range(dim[1]+1,self.dim))] , 0)                                                  
            return np.dot(temp * self.data[dim[0]][ind[dim[0]],:], self.data[dim[1]][ind[dim[1]],:].T )
        elif len(dim) == 3:
            res = np.zeros((len(ind[dim[0]]), len(ind[dim[1]]), len(ind[dim[2]])) ) #initialisation du tableau
    ...
    def __setitem__(self, key, value):     
        ...
        key = list(key)
        if len(key) < self.dim: key = list(key)+[slice(None) for i in range(self.dim - len(key))] #si il manque des variables on prend tout l'intervale
        if isinstance(value, list): value = np.ndarray(value)            
        ...
        for ii, kk in enumerate(key):                          
            if isinstance(kk, list) or isinstance(kk,np.ndarray): 
                ...
                if len(key[ii]) == 1: key[ii] = key[ii][0]
            if not(isinstance(kk, (Number, np.floating))): dim.append(ii)  
  
        if len(dim) == 0: #if only one element is modified in the global array. Value must be an int or a float
            ...
            if isinstance(value, np.ndarray): value = value.reshape(-1)
            FFadd = SeparatedZeros(self.shape) #SeparatedArray to add                       
            old_value = np.dot(self.data[dim[0]][key[dim[0]]], np.prod([self.data[dd][key[dd], :] for dd in list(range(dim[0]))+list(range(dim[0]+1,self.dim))] , 0) ) #old values            
            for dd, kk in enumerate(key): 
                if dd == dim[0]: FFadd.data[dd][kk] = np.c_[value - old_value]
                else: FFadd.data[dd][kk] = 1.                            
            ...
            if isinstance(value, (Number, np.floating)): 
                ...
                if len(value) != len(self):
                    if len(value) ==  len(dim):                            
                        nbt = value.nbTerm()
                        value = SeparatedArray( [ value.data[dim.index(d)] if d in dim \
                                                  else np.ones((1,nbt)) for d in range(len(self))] )
...
#        self.data = (self+FFadd).data
    ...
    def __init__(self, listD, N=0):
        ...
        elif isinstance(listD, SeparatedArray): self.data = [Fd.copy() for Fd in listD.data]
    ...
    def __neg__(self): #retourne l'opposé 
        return SeparatedArray([ np.c_[-self.data[0]] ] + [ np.c_[self.data[dd]] for dd in range(1,self.dim) ])
...
#                str(self.data) # affiché avec la fonction print        
    ...
    def __sub__(self, A): #soustraction
        if isinstance(A, SeparatedArray) == 1:
            if self.dim == A.dim:   
                return SeparatedArray([ np.c_[self.data[0],-A.data[0]] ] + [ np.c_[self.data[dd],A.data[dd]] for dd in range(1,self.dim) ])
            else: raise NameError('Dimensions doesnt match')
        elif isinstance(A, Number) == 1:  
            if A == 0: return self
            else: return self+SeparatedArray(-A,self.shape)
    ...
    def multiply(self,a):
        ...
        if isinstance(a, Number):       
            return SeparatedArray([ np.c_[a*self.data[0]] ] + [ np.c_[self.data[dd]] for dd in range(1,self.dim) ])
        elif isinstance(a, SeparatedArray): #implémentation rapide à optimiser
            res = 0
            for ii in range(self.nbTerm()):
                for jj in range(a.nbTerm()):
                    res += SeparatedArray([np.c_[self.data[dd][:,ii]*a.data[dd][:,jj]] for dd in range(self.dim)])
            return res
        else: return NotImplemented
    ...
    def tensorProd(self, FF, modify = False): # à vérifier dans le cas où le nombre de terme est différent de 1 pour les deux objets SeparatedArray
        """
        ...
        """
        if FF==0 and modify != True: return 0
        if isinstance(FF, Number):       
            res = [ np.c_[FF*self.data[0]] ] + [ np.c_[self.data[dd]] for dd in range(1,self.dim) ]
        elif isinstance(FF, SeparatedArray):
            nbTermFF = FF.nbTerm()
            res = [np.tile( self.data[ii] , ((1, nbTermFF))) for ii in range(self.dim)]
            res += [np.hstack([ff[:,[col]]*np.ones(self.nbTerm()) for col in range(nbTermFF)]) for ff in FF.data] 
        else: return NotImplemented
        if modify == True:
            self.data = res ; self.updateShape()
            return self
        else: 
...
#            for var in range(nbvar): CC[var].reduction(max_iter,max_norm_err)                
    ...
    def reduction_multivar(self, nbvar = 0, max_iter = 1000, max_norm_err = 1e-4, max_iter_RS = 5, max_norm_RS = 1e-5):    
        dim = self.dim 
        NN=np.array(self.shape)//nbvar
...
#            print("iter : " + str(iter))
            if iter != 0: R_iter = SeparatedArray(R)                  
            else: R_iter = 0
            R += SeparatedArray(1,NN*nbvar) ; nbR+=1
            err_RS = 1
            comp_nR = 0      
    ...
    #            nbR += 1            
                    ...
                    for d1 in range(dim):                    
                        prod_aux = 1; prod_aux_3 = 1
                        for d2 in list(range(d1))+list(range(d1+1,dim)): 
                            prod_aux = prod_aux * np.dot(R.data[d2][var*NN[d2]:(var+1)*NN[d2]].T,R.data[d2][var*NN[d2]:(var+1)*NN[d2]])   
                            prod_aux_3 = prod_aux_3 * (np.dot(R.data[d2][var*NN[d2]:(var+1)*NN[d2]].T,CC.data[d2][var*NN[d2]:(var+1)*NN[d2]]))                        
                        Vec = np.dot(CC.data[d1][var*NN[d1]:(var+1)*NN[d1]], prod_aux_3.T)                    
                        prod_aux = linalg.inv(prod_aux)                            
#                        R.data[d1][var*NN[d1]:(var+1)*NN[d1]] = np.dot(Vec,prod_aux.T)
                        R.data[d1][var*NN[d1]:(var+1)*NN[d1]] = np.dot(Vec,prod_aux)             
    
#                    R.regularise()
                
                err_RS = (R-R_old).norm(nbvar = nbvar)/err_0
#                print(err_RS)
                if comp_nR > max_iter_RS: break            
...
#            print(np.dot(prod_aux,prod_aux.T))
            ...
            print(str(iter)+' - '+str(comp_nR)+' - '+str(norm_err) +' - '+ str(err_RS))
            if norm_err < max_norm_err: break                
...
#                print(err_RS)
            ...
            CC = CC-R          
            if self.data ==[]: self.data = list(R.data)
            else: self.data = [ np.c_[self.data[dd],R.data[dd]] for dd in range(self.dim) ] 
...
#            print(norm_err)
    ...
    def reduction2D(self, max_norm_err = 1e-4, pod = 1): #ne marche pas si dim>2
        if self.dim==2:   
            if pod == 0:
                if self.shape[0]<self.shape[1]:            
                    ...
                    self.data[1] = CC ; self.data[0] = np.identity(np.shape(CC)[1])
                else:
                    CC =  np.dot(self.data[0],self.data[1].T)
                    self.data[0] = CC ; self.data[1] = np.identity(np.shape(CC)[1])
            else:
                CC = np.dot(self.data[0],self.data[1].T)
        ...
        else: np.disp('Warning: reduction_POD dont work if dim != 2') 
        return self
    ...
    def regularise(self,nbvar=1):
        ...
        for var in range(nbvar):        
            for dd in range(1,self.dim):       
                norme_dd = np.sum(np.sqrt(np.dot(self.data[dd][var*NN[dd]:(var+1)*NN[dd],:].T,self.data[dd][var*NN[dd]:(var+1)*NN[dd],:]) ))
                self.data[0][var*NN[dd]:(var+1)*NN[dd],:] = self.data[0][var*NN[dd]:(var+1)*NN[dd],:]* norme_dd
                self.data[dd][var*NN[dd]:(var+1)*NN[dd],:] = self.data[dd][var*NN[dd]:(var+1)*NN[dd],:]/ norme_dd
    ...
    def squareNorm(self, nbvar=1):
        ...
        for var in range(nbvar):
            ...
            for dd in range(self.dim): 
                prod_aux = prod_aux * np.dot(self.data[dd][var*self.shape[dd]//nbvar:(var+1)*self.shape[dd]//nbvar,:].T , self.data[dd][var*self.shape[dd]//nbvar:(var+1)*self.shape[dd]//nbvar,:]) 
...
#        return SeparatedArray([self.data[dd][var*self.shape[dd]//nbvar:(var+1)*self.shape[dd]//nbvar,:] for dd in range(self.dim)])
    ...
    def swap_space(self, ind1, ind2=0): 
        ...
        if isinstance(ind1, int):
            if ind1 == ind2: return self
            ind1 = range(ind1)+[ind2]+range(ind1+1,ind2)+[ind1]+range(ind2+1,self.dim)
...
def ConvertArraytoSeparatedArray(FF): #ne marche que pour les fonctions 2D    
    if isinstance(FF, list):
        dim = len(np.shape(FF.data[0])) ; nbvar = len(FF)
        temp = [ConvertArraytoSeparatedArray(FF.data[var]) for var in range(nbvar)]
        return SeparatedArray([np.vstack( [temp[var][dd] for var in range(nbvar)] ) for dd in range(dim)])                
    ...
    else: raise NameError('Not implemented error')     
...
def MergeSeparatedArray(F1, F2):    
    ...
    return SeparatedArray([np.vstack((F1.data[dd],F2.data[dd])) for dd in range(F1.dim)])    
