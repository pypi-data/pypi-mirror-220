...
class Curation(Command):
    ...
    def _classify_transcripts(self, genes):
        ...
        1: high confidence
        2: good confidence
        3: good confidence, supported by one evidence type
        4: moderate confidence
        5: high to moderate confidence, with penalty on structure
        6: bad confidence
        ...
        for gene in genes:
            for trans in gene.lTranscripts:
                ...
                # category 2
                elif (aed_trans < 0.1 and trans.best_bx_evidence[1] < 0.5 ) \
    ...
    def run(self):
        ...
        Graphics.plot_curation_scatter_hist(graph_cats
            ...
            legend=[f'cat1: high confidence (# {len(categories[0])})',\
                    ...
                    f'cat3: good confidence, supported by one evidence type (# {len(categories[2])})',\
                    f'cat4: moderate confidence (# {len(categories[3])})',\
                    f'cat5: high to moderate confidence, with penalty on structure (# {len(categories[4])})',\
