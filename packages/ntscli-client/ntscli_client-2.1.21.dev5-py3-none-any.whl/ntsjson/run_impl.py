...
def run_impl(
    ...
    with stateful_session_mgr(**dict(configuration=configuration, **session_target.to_dict())) as session:
        ...
        if not force_run_peripheral_tests and (should_remove_peripheral_tests or force_skip_peripheral_tests):
            skip_peripheral_testcases(chosen_plan=session.plan_request)
