...
class Listener:
    ...
    def log_message(self, message):
        ...
        if self.console_flag:
            ...
            if (datetime.datetime.now() - self.last_log) > datetime.timedelta(seconds=1):
                self._log_records()
    ...
    def close(self):
        ...
        if self.final_status == "FAIL":
            self._send_status_log_files(
                self.path_log, self.path_report, self.path_xml, "FAILURE", "Ha ocurrido un error en el Robot"
            )
        elif self.errors:
            self._send_status_log_files(
                self.path_log,
            ...
            self._send_status_log_files(
                self.path_log, self.path_report, self.path_xml, "SUCCESS", "Robot ejecutado con exito"
            )
