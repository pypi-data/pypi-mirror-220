...
def test_exception_with_only_logger_object():
    try:
        ...
        stack_trace = str(traceback.format_exception(type(e), e, e.__traceback__))
...
def test_check_Function():
    ...
    conn = get_connection()
    cursor = conn.cursor()
    sql = f"""SELECT component_id FROM logger.logger_table WHERE payload = '{'{"payload": "check python", "component_id": 101, "a": 5, "b": 6, "severity_id": 700, "function": "test_check_Function"}'}' ORDER BY timestamp DESC LIMIT 1;"""
    cursor.execute(sql)
    result = cursor.fetchone()
    assert result[0] == 101
    ...
    sql = f"""SELECT component_id FROM logger.logger_table WHERE payload = '{'{"component_id": 101, "payload": "check python", "return": 9, "severity_id": 800, "function": "test_check_Function"}'}' ORDER BY timestamp DESC LIMIT 1;"""
    cursor.execute(sql)
    result = cursor.fetchone()
    assert result[0] == 101
