...
class Chat():
    ...
    def forge(self, *messages: Union[user_msg, assistant_msg]):
        ''' 伪造对话内容 '''
        for x in messages:
            if isinstance(x, assistant_msg):
                self._messages.add_many({"role": "assistant", "content": str(x)})
            else:
                self._messages.add_many({"role": "user", "content": str(x)})
