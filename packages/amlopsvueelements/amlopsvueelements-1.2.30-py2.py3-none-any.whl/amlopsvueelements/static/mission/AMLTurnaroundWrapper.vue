                  ...
                  <div class="flex items-center justify-between w-full">
                    <div class="flex items-center shrink-0 whitespace-nowrap h-[22px]">
                      <div class="flex items-center w-full">
                        <img
                          :class="[
                            $style['ops-aml-turnaround__content__delete-icon'],
                            { 'opacity-50 cursor-not-allowed': isDisabledService(service.service) }
                          ]"
                        ...
                        <UTooltipWrapper>
                          <template #target>
                            <ULabel
                              text-class="mb-0 text-sm truncate max-w-[7rem]"
                              :required="false"
                              :label-text="findServiceById(service.service).name"
                            />
                          </template>
                          <template #popper> {{ findServiceById(service.service).name }} </template>
                        </UTooltipWrapper>
                      </div>
                    </div>
                    <div
...
import { getAxiosBaseUrl, getHost, getImageUrl } from '@/helpers'
...
const onRemoveService = (serviceIndex: number, serviceId: string) => {
  if (isDisabledService(serviceId)) return
