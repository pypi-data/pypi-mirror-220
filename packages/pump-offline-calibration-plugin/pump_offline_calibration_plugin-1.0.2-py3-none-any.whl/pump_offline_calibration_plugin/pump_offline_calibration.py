...
def introduction() -> None:
    ...
    click.clear()
    click.echo(
        """This routine will calibrate the pumps on your current Pioreactor based on a set of offline measurements. You'll need:
    ...
    For a given pump, measure a set of volumes expelled from fixed runs of 0.5-1.5 seconds.

    Note: make sure the PWM and duty cycle settings are consistent!
    ...
    with local_persistant_storage("pump_calibrations") as cache:
        while True:
            ...
            if name == "":
                ...
                if click.confirm(
                    click.style(
                        "❗️ Name already exists. Do you wish to overwrite?", fg="green"
                    )
                ):
                    break
            elif name == "current":
...
def run_tests(
    ...
    for i, duration in enumerate(durations_to_test):
        while True:
            ...
            r = click.prompt(
                click.style(
                    "Enter amount of water expelled (g or ml), or REDO", fg="green"
                ),
...
def publish_to_leader(name: str) -> bool:
    ...
    if not success:
        click.echo(
            f"❌ Could not publish on leader at http://{leader_address}/api/calibrations"
        )
    return success


def pump_offline_calibration(min_duration: float, max_duration: float) -> None:
    unit = get_unit_name()
    experiment = get_latest_experiment_name()

    logger = create_logger("pump_calibration", unit=unit, experiment=experiment)
    logger.info("Starting offline pump calibration.")
    ...
    with publish_ready_to_disconnected_state(unit, experiment, "pump_calibration"):
        ...
        while is_ready:
            ...
            is_ready = click.confirm(
                click.style(
                    "Do you want to change the frequency or duty cycle?", fg="green"
                ),
                prompt_suffix=" ",
                default=False,
            )

        click.echo("Enter comma-separated list of test times (in seconds)")
        click.echo("Example: 0.5,0.5,1.0,1.1,1.5,1.5")
        durations_str = click.prompt("Durations: ", type=str)
        durations = [float(t) for t in str.split(durations_str, ",")]
        ...
        click.echo(
            f"slope={slope:0.3f} ± {std_slope:0.3f}, bias={bias:0.3f} ± {std_bias:0.3f}"
        )
...
def change_current(name: str) -> bool:
    with local_persistant_storage("pump_calibrations") as all_calibrations:
        ...
        with local_persistant_storage(
            "current_pump_calibration"
        ) as current_calibrations:
...
def list_():
    ...
    with local_persistant_storage("pump_calibrations") as c:
        for name in c.iterkeys():
            try:
                cal = decode(
                    c[name], type=structs.subclass_union(structs.PumpCalibration)
                )
                click.secho(
...
@click.group(invoke_without_command=True, name="pump_offline_calibration")
@click.pass_context
@click.option("--min-duration", type=float)
@click.option("--max-duration", type=float)
def click_pump_offline_calibration(ctx, min_duration, max_duration):
    """
    Calibrate a pump using offline measurements
    """
    if ctx.invoked_subcommand is None:
        ...
        pump_offline_calibration(min_duration, max_duration)


@click_pump_offline_calibration.command(name="display")
...
@click_pump_offline_calibration.command(name="change_current")
...
@click_pump_offline_calibration.command(name="list")
def click_list():
    """
...
@click_pump_offline_calibration.command(name="publish")
@click.argument("name", type=click.STRING)
...
if __name__ == "__main__":
    click_pump_offline_calibration()
