...
class _Psycopg2Adapter(db.DBAPIAdapter):
    ...
    def set_search_path(self, cnx, schema):
        if schema:
            ...
            schemas_sql = sql.SQL(",").join((sql.Identifier(s) for s in schemas))
            query = sql.SQL("SET search_path TO {};").format(schemas_sql)
...
class _PGAdvFuncHelper(db._GenericAdvFuncHelper):
    ...
    def sql_create_sequence(self, seq_name):
        return sql.SQL("CREATE SEQUENCE {};").format(sql.Identifier(seq_name))

    def sql_restart_sequence(self, seq_name, initial_value=1):
        return sql.SQL("ALTER SEQUENCE {} RESTART WITH {};").format(
            sql.Identifier(seq_name), initial_value
        )

    def sql_sequence_current_state(self, seq_name):
        return sql.SQL("SELECT last_value FROM {};").format(sql.Identifier(seq_name))

    def sql_drop_sequence(self, seq_name):
        return sql.SQL("DROP SEQUENCE {};").format(sql.Identifier(seq_name))

    def sqls_increment_sequence(self, seq_name):
        return (sql.SQL("SELECT nextval({});".format(sql.Identifier(seq_name))),)
    ...
    def sql_temporary_table(self, table_name, table_schema, drop_on_commit=True):
        if not drop_on_commit:
            return sql.SQL("CREATE TEMPORARY TABLE {} ({});").format(
                sql.Identifier(table_name), sql.Identifier(table_schema)
            )
        return sql.SQL("CREATE TEMPORARY TABLE {} ({}) ON COMMIT DROP;").format(
    ...
    def create_database(
        ...
        if owner:
            query += sql.SQL(" WITH OWNER={}").format(sql.Identifier(owner))
        if template:
            query += sql.SQL(" TEMPLATE {}").format(sql.Identifier(template))
        dbencoding = dbencoding or self.dbencoding
        if dbencoding:
            query += sql.SQL(" ENCODING={}").format(sql.Identifier(dbencoding))
        cursor.execute(query)
    ...
    def create_schema(self, cursor, schema, granted_user=None):
        ...
        if granted_user is not None:
            query += sql.SQL(" AUTHORIZATION {}").format(sql.Identifier(granted_user))
        query += sql.SQL(";")
        cursor.execute(query)
    ...
    def create_language(self, cursor, extlang):
        ...
        if cursor.fetchall():
            ...
            cursor.execute(
                sql.SQL("CREATE LANGUAGE {}").format(sql.Identifier(extlang))
            )
...
"""
    ...
    def sql_grant_user_on_fti(self, user):
        return sql.SQL("GRANT ALL ON appears TO {};").format(sql.Identifier(user))
