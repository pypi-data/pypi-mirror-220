...
mims_cutoff_list = {"PA_Level1": 10.558, "PA_Level2": 15.047, "PA_Level3": 19.614, "PA_Level4": 25.0, "PA_Level5": 25.0}
...
def get_mims_summary_after_prompt_wearstate(wear_state, sec_after, prompt_time, df_mims_swan_minute_combined,
            ...
            if mims_sample_num > 0:
                ...
                if wear_state == "Wear":
                    if mims_value < mims_cutoff_list["PA_Level1"]:
                        ...
                        pa_level4_min += 1
                    elif mims_value >= mims_cutoff_list["PA_Level4"]:
                        pa_level5_min += 1
                    else:
                        pass
    else:
    ...
    return mims_summary_wearstate, mims_wearstate_min, pa_level1_min, pa_level2_min, pa_level3_min, pa_level4_min, pa_level5_min
...
def match_feature(prompt_local_datetime_series, df_mims_combined, df_mims_swan_minute_combined, date_in_study):
        ...
        if df_mims_combined.shape[0] == 0:
            ...
            for sec_after in sec_after_list:
                ...
                for wear_state in ["Wear", "Sleep", "Nonwear"]:
                    mims_summary_wearstate, min_wearstate, pa_level1_min, pa_level2_min, pa_level3_min, pa_level4_min, pa_level5_min = get_mims_summary_after_prompt_wearstate(
                        wear_state,
                        sec_after,
                        prompt_time,
    ...
    for sec_after in sec_after_list:
        ...
        df = pd.concat([df, pa_level1_wear_after_df[col].rename("SEDENTARY_MIN_WEAR_" + str(sec_after // 60) + "MIN_AFTER")],
                       axis=1)
        df = pd.concat(
            [df, pa_level2_wear_after_df[col].rename("LIGHT_MIN_WEAR_" + str(sec_after // 60) + "MIN_AFTER")],
            axis=1)
        df = pd.concat(
            [df, pa_level3_wear_after_df[col].rename("MODERATE_MIN_WEAR_" + str(sec_after // 60) + "MIN_AFTER")],
            axis=1)
        df = pd.concat(
            [df, pa_level4_wear_after_df[col].rename("VIGOROUS_MIN_WEAR_" + str(sec_after // 60) + "MIN_AFTER")],
            axis=1)
        df = pd.concat(
            [df, pa_level5_wear_after_df[col].rename("VIGOROUS_MIN_WEAR_" + str(sec_after // 60) + "MIN_AFTER")],
            axis=1)
