...
def lmk_loss(opt_lmks, target_lmks, lmk_mask):
    # opt_lmks, target_lmks = scale_lmks(opt_lmks, target_lmks, image_size)
    diff = torch.pow(opt_lmks/512 - target_lmks/512, 2)
...
def oval_lmk_loss(opt_lmks, target_lmks, lmk_mask):
    oval_ids = [i for i in range(17)]
    # opt_lmks, target_lmks = scale_lmks(opt_lmks, target_lmks, image_size)
    diff = torch.pow(opt_lmks[:, oval_ids, :]/512 - target_lmks[:, oval_ids, :]/512, 2)
...
def get_gaussian_pyramid(levels, kernel_size=7, sigma=9.0):
    pyramid = []
    for k, level in enumerate(reversed(levels)):
        image_size, iters = level
        size = [int(image_size[0]), int(image_size[1])]
        pyramid.append((iters, size, image_size))
