...
async def print_voices(*, proxy: str) -> None:
...
async def run_tts_standalone(media_file, write_subtitles, text="", voice="") -> None:
    ...
    with open(media_file, "wb")  as audio_file:
    ...
    sub_file: Union[TextIOWrapper, TextIO] = (
        open(write_subtitles, "w", encoding="utf-8")
    )
    with sub_file:
        sub_file.write(subs.generate_subs(10))
...
async def amain() -> None:
    ...
    if args.list_voices:
        await print_voices(proxy=args.proxy)
        sys.exit(0)
