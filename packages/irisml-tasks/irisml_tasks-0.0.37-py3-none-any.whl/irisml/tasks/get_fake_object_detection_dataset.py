...
class Task(irisml.core.TaskBase):
    ...
    def execute(self, inputs):
        ...
        def _random_box():
            cx = rng.random()
            cy = rng.random()
            w = rng.uniform(0, min(1 - cx, cx))
            h = rng.uniform(0, min(1 - cy, cy))
        ...
        if self.config.targets:
            ...
            targets = [torch.tensor([[rng.randrange(self.config.num_classes), *_random_box()] for _ in range(rng.randrange(self.config.num_max_boxes + 1))]).reshape(-1, 5)
