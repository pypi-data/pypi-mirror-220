...
def login(username, password, mfa_token: str = "", token: str = None):
    ...
    if password:
        if mfa_token:
            lg.login(
                str_user=username, str_password=password, mfa_token=mfa_token, app=app
            )
        else:
            lg.login(str_user=username, str_password=password, app=app)
        sys.exit(0)
