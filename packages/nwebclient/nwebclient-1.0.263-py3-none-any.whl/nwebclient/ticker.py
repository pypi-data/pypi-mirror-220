...
class Process(b.Base, b.Named):
    ...
    def toHtml(self, params={}):
        info = b.StrBuffer()
        self.printInfo(p=info)
        i = id(self)
        return """
          <div class="python_object process">Process """+self.className()+" "+self.htmlContent(params)+self.name+self.getHtmlTree()+'<hr /><pre>'+str(info)+"ID: "+str(i)+"""</pre></div>
        """
...
class CmdOps(Process):
    ...
    def toHtml(self, params={}):
        if 'exec' in params:
            ...
            return "Done."
        return '<form><input type="text" name="exec" /><input type="submit" name="submit" value="Exec" /></form>'
...
class NWebJobFetch(Ticker):
    ...
    def download(self, doc):
        ...
        self.cpu.jobs.append(data)
        if 'keep' in data and data['keep'] == True:
...
class WebProcess(Process):
    ...
    def index(self):
        ...
        s.ul(['<a href="registry">Ticker-Registry</a>','<a href="root">Root</a>'])
    ...
    def registry(self):
        ...
        s = '<ul>'
        for entry_point in iter_entry_points(group='nweb_ticker', name=None):
            s +='<li>'+str(entry_point)+'</li>'
        s += '</ul>'
        return s
        #from pkg_resources import iter_entry_points
        #available_methods = []
    ...
    def nw(self, path):
        from flask import request
        obj = util.load_class(path, create=True)
        if isinstance(obj, b.PublicWebObject):
            w = self.addChild(b.WebObject(obj, request.args.to_dict()))
            return w.page()
        else:
