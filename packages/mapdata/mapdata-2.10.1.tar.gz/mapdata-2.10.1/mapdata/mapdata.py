...
class PlotDialog(object):
	def __init__(self, parent, column_specs):
		...
		self.type_sel = ttk.Combobox(ctrl_frame, state="readonly", textvariable=self.type_var, width=20,
				values=["Box plot", "Breaks groups", "Breaks optimum", "Category counts", "Empirical CDF", "Histogram", "Line plot", "Normal Q-Q plot", "Scatter plot", "Y range plot"])
	...
	def q_redraw(self, get_data=True, *args):
		...
		plot_type = self.type_var.get()
		can_redraw = (plot_type in ("Category counts", "Empirical CDF", "Normal Q-Q plot", \
				"Breaks groups", "Breaks optimum", "Histogram") and self.x_var.get() != '') \
				or (plot_type in ("Scatter plot", "Line plot", "Box plot", "Y range plot") \
				and self.x_var.get() != '' and self.y_var.get() != '')
		if can_redraw:
			self.plotfig.clear()
			self.plot_axes = self.plotfig.add_subplot(111)
			self.plotfig_canvas.draw()
