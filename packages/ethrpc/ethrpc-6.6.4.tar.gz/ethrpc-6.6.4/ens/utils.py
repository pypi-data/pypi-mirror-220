...
def label_to_hash(label: str) -> HexBytes:
    ...
    return Ethrpc().keccak(text=label)
...
def normal_name_to_hash(name: str) -> HexBytes:
    ...
    if not is_empty_name(name):
        ...
        for label in reversed(labels):
            ...
            node = Ethrpc().keccak(node + labelhash)
    return node
...
# borrowed from similar method at `ethrpc._utils.abi` due to circular dependency
