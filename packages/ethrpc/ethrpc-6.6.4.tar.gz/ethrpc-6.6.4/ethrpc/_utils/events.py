...
if TYPE_CHECKING:
    ...
    from ethrpc._utils.filters import (  # noqa: F401
        AsyncLogFilter,
        LogFilter,
    )
...
class EventFilterBuilder(BaseEventFilterBuilder):
    def deploy(self, w3: "Ethrpc") -> "LogFilter":
        if not isinstance(w3, ethrpc.Ethrpc):
            raise ValueError(f"Invalid ethrpc argument: got: {w3!r}")
...
class AsyncEventFilterBuilder(BaseEventFilterBuilder):
    async def deploy(self, async_w3: "AsyncWeb3") -> "AsyncLogFilter":
        if not isinstance(async_w3, ethrpc.AsyncWeb3):
            raise ValueError(f"Invalid ethrpc argument: got: {async_w3!r}")
