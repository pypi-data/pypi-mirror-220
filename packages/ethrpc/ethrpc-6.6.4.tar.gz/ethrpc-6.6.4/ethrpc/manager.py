...
class RequestManager:
    logger = logging.getLogger("ethrpc.RequestManager")
    ...
    def __init__(
        ...
        if middlewares is None:
            middlewares = (
                ...
                else self.default_middlewares(cast("Ethrpc", w3))
            )

        self.middleware_onion = NamedElementOnion(middlewares)

    w3: Union["AsyncWeb3", "Ethrpc"] = None
    _provider = None
    ...
    @staticmethod
    def default_middlewares(w3: "Ethrpc") -> List[Tuple[Middleware, str]]:
        """
    ...
    def _make_request(
        ...
        request_func = provider.request_func(
            cast("Ethrpc", self.w3), cast(MiddlewareOnion, self.middleware_onion)
        )
