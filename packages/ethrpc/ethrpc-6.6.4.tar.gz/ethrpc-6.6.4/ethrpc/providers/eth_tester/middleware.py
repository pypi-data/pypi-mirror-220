...
def guess_from(w3: "Ethrpc", _: TxParams) -> ChecksumAddress:
    if w3.eth.coinbase:
...
@curry
def fill_default(
    field: str, guess_func: Callable[..., Any], w3: "Ethrpc", transaction: TxParams
) -> TxParams:
...
async def async_ethereum_tester_middleware(  # type: ignore
    make_request, ethrpc: "AsyncWeb3"
) -> Middleware:
    middleware = await async_construct_formatting_middleware(
        request_formatters=request_formatters, result_formatters=result_formatters
    )
    return await middleware(make_request, ethrpc)
