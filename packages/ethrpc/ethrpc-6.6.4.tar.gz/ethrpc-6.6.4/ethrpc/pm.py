...
# is not automatically available on a ethrpc instance. To use the `PM` module,
# please enable the package management API on an individual ethrpc instance.
#
# >>> from ethrpc import Ethrpc, IPCProvider
# >>> w3 = Ethrpc(IPCProvider(...))
# >>> w3.pm
...
class ERC1319Registry(ABC):
    ...
    accessed directly, but rather through the methods on ``ethrpc.pm``.
    ...
    with `ethrpc.pm.PM`. Any implementation specific logic should be
    handled in a subclass.
    """

    @abstractmethod
    def __init__(self, address: Address, w3: Ethrpc) -> None:
        """
        Initializes the class with the on-chain address of the registry, and a ethrpc
        instance connected to the chain where the registry can be found.

        Must set the following properties...

        * ``self.registry``: A `ethrpc.contract` instance of the target registry.
        * ``self.address``: The address of the target registry.
        * ``self.w3``: The *ethrpc* instance connected to the chain where the
    ...
    @classmethod
    @abstractmethod
    def deploy_new_instance(cls: Type[T], w3: Ethrpc) -> T:
        """
        Class method that returns a newly deployed instance of ERC1319Registry.

        * Parameters:
            * ``w3``: Ethrpc instance on which to deploy the new registry.
        """
        pass
...
class SimpleRegistry(ERC1319Registry):
    ...
    def __init__(self, address: ChecksumAddress, w3: Ethrpc) -> None:
    ...
    @classmethod
    def deploy_new_instance(cls, w3: Ethrpc) -> "SimpleRegistry":
...
class PM(Module):
    ...
    def set_registry(self, address: Union[Address, ChecksumAddress, ENS]) -> None:
        """
        Sets the current registry used in ``ethrpc.pm`` functions that read/write
    ...
    def deploy_and_set_registry(self) -> ChecksumAddress:
        ...
        To tie your registry to an ENS name, use ethrpc's ENS module, ie.
    ...
    def release_package(
        ...
        Requires ``ethrpc.PM`` to have a registry set. Requires
        ``ethrpc.eth.default_account`` to be the registry owner.
    ...
    def _validate_set_registry(self) -> None:
        try:
            ...
            raise EthPMException(
                "ethrpc.pm does not have a set registry. "
                "Please set registry with either: "
                "ethrpc.pm.set_registry(address) or "
                "ethrpc.pm.deploy_and_set_registry()"
            )
        if not isinstance(self.registry, ERC1319Registry):
            raise EthPMException(
                "ethrpc.pm requires an instance of a subclass of ERC1319Registry "
                "to be set as the ethrpc.pm.registry attribute. Instead found: "
    ...
    def _validate_set_ens(self) -> None:
        if not self.w3:
            raise InvalidAddress(
                "Could not look up ENS address because no ethrpc connection available"
            )
        elif not self.w3.ens:
            raise InvalidAddress(
                "Could not look up ENS address because ethrpc.ens is not set up"
            )
